...
my.data_temp$description <- case_when(
  ...
  my.data$`Gene names` %in% pero$`Gene names` ~ 4,
...
my.data_temp$label <- case_when(
  ...
  my.data_temp$sig_MITOS_OCIAD1 == TRUE & my.data_temp$description == 4 ~ my.data_temp$`Gene names`,
...
my.data_temp$color <- case_when(
  ...
  my.data$`Gene names` %in% pero$`Gene names` ~ '#A020F0', 
...
v1 <- ggplot(my.data_temp, aes(x = FC_MITOS_OCIAD1, y = -log10(p_OCIAD1_MITOS), shape = sig_MITOS_OCIAD1, color = description)) +
  ...
  geom_point(data = filter(my.data_temp, description == 4), stroke = 1.2, alpha = 0.7) +
  ...
  scale_color_manual(labels = c('Non-mitochondrial', 'Other mitochondrial', 'Fatty acid oxidation', 'Protein import', 'Peroxisomal'), values = c('darkgrey', '#76EE35', 'blue', 'darkorange', '#A020F0')) +
