...
pvalue <- lfq |>
    mutate("p_OCIAD1_MITOS" = apply(lfq, 1, ttest,
                                    grp1=grep("KO", colnames(lfq)), 
                                    grp2=grep("WT", colnames(lfq)))) |>
    mutate(sig_MITOS_OCIAD1 = ifelse(p_OCIAD1_MITOS < cutoff.p, TRUE, FALSE)) |>
    relocate(protein.ids, .before = 1)

# Histogram of p-values
ggplot(data = pvalue, aes(x = p_OCIAD1_MITOS, fill=sig_MITOS_OCIAD1)) + 
    geom_histogram(bins = 100) + ggtitle("MITOS - p.value")
...
means <- pivot_wider(means, names_from = group, values_from = mean) |>
    mutate(FC_MITOS_OCIAD1 = MITOS_OCIAD1_KO - MITOS_OCIAD1_WT)

# Preparing data for volcano plot
FC <- merge(means, lfq, by = "protein.ids")

final_table <- merge(FC, pvalue[,c("protein.ids", "p_OCIAD1_MITOS", "sig_MITOS_OCIAD1")], by = "protein.ids") |>
    select(-MITOS_OCIAD1_KO, -MITOS_OCIAD1_WT) |>
