...
pred_test_na <- predict(rforest, test_data_na[,1:11])
...
predictions <- predict(rforest, protein.groups[missing,1:11])
...
```{r echo=FALSE}
DT::datatable(rbind(rf_measures, kNN_measures), options=list(searching=FALSE, paging=FALSE, info=FALSE))
```
