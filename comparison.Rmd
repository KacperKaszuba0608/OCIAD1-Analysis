...
```{r}
df <- lfq[which(miss_type$miss == 'complete'),-c(3,6)]
...
# Preparing data for volcano plot
FC <- merge(means, lfq2[which(miss_type$miss == 'complete'),], by = 'protein.ids')
...
```{r}
# Preparing data for calculations
lfq4 <- cbind.data.frame(eigen_df2[which(miss_type$miss == 'complete'),], 
                        protein.ids=protein.groups$`Protein IDs`[rows.to.keep][which(miss_type$miss == 'complete')],
                        gene = protein.groups$`Gene names`[rows.to.keep][which(miss_type$miss == 'complete')])
lfq4_long <- lfq4 |>
    tidyr::pivot_longer(1:4, names_to = 'sample', values_to = 'LFQvalue')
...
# Preparing data for volcano plot
FC <- merge(means, lfq4[which(miss_type$miss == 'complete'),], by = 'protein.ids')
