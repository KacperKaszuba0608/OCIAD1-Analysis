...
assign_missing <- function(protein.ids, condition, lfq_intensity) {
    ...
    cond_id <- seq(1,nrow(test), by=3)
    test2 <- test[cond_id,]
    
    test3 <- purrr::map(1:nrow(test2), function(i) {
        if (all(test2[i,] == 'all_NA')) {
            rep(NA, occur)
        } else if ((test2[i,1] == 'complete' & test2[i,2] == 'all_NA') | (test2[i,2] == 'complete' & test2[i,1] == 'all_NA')) {
            rep('MNAR', occur)
        } else if ((test2[i,1] == 'MAR' & test2[i,2] == 'MNAR') | (test2[i,2] == 'MAR' & test2[i,1] == 'MNAR')) {
            rep(NA, occur)
        } else if (all(test2[i,] == 'MAR')) {
            rep('MAR', occur)
        } else if (all(test2[i,] == 'MNAR')) {
            rep('MNAR', occur)
        } else if (all(test2[i,] == 'complete')) {
            rep('complete', occur)
        } else {
            rep(NA, occur)
        }
...
df_ml.to.models <- df_ml |>
    dplyr::select(-prot.id, -Sample, -sampletype) |> dplyr::select(1:10, 12, 13, 11) |>
...
df_ml <- df_ml |>
    dplyr::select(-prot.id, -Sample) |> dplyr::select(1:11, 13, 14, 12) |>
...
imputed_RF <- df_ml
imputed_RF[missing,14] <- predictions$predictions
...
# data frame with predicted LFQ values by random forest model
imputed_kNN <- df_ml
imputed_kNN[missing,14] <- predictions2
...
to_plot_KO22.0$ludovic[ludovic_ids] <- LFQ_KO_ludovic$KO_22
...
to_plot_KO22.1$ludovic[ludovic_ids] <- LFQ_KO_ludovic.standard$KO_22
...
to_plot_KO22.2$ludovic[ludovic_ids] <- LFQ_KO_ludovic.minmax$KO_22
...
to_plot_KO22.3$ludovic[ludovic_ids] <- LFQ_KO_ludovic.med$KO_22
...
to_plot_KO22.4$ludovic[ludovic_ids] <- LFQ_KO_ludovic.mad$KO_22
...
to_plot_KO22.5$ludovic[ludovic_ids] <- LFQ_KO_ludovic.lm$KO_22
...
to_plot_KO22.6$ludovic[ludovic_ids] <- LFQ_KO_ludovic.vsn[,1]
...
to_plot_KO22.7$original[raw_ids] <- LFQ_KO.eigen[,1]
to_plot_KO22.7$ludovic[ludovic_ids] <- LFQ_KO_ludovic.eigen[,1]
...
rm(to_plot_KO22.0, to_plot_KO22.1, to_plot_KO22.2, to_plot_KO22.3, to_plot_KO22.4,
   to_plot_KO22.5, to_plot_KO22.6, to_plot_KO22.7, ludovic_ids, raw_ids)
```
...
output$lfq.ko.22.Plot <- renderPlotly({
    ...
    filtered_data <- to_plot_KO22 |>
        ...
        tidyr::pivot_longer(2:6, names_to='Type', values_to = 'LFQ_KO_22')
