...
shift_dist_impute = function(df, width = 0.3, downshift = 1.8, seed = 7694) {
...
imputation <- function(rf_model, df_to_model, df_ml) {
    ...
    mar_prots <- which(df_to_model$missingness == 3)
    mnar_prots_long <- which(df_to_model$missingness == 4)
...
# extracting missing rows - can be useful later
...
imputed_RF <- df_ml
imputed_RF[missing,15] <- predictions$predictions
...
# predict the values for NA rows
predictions2 <- predict(kNN.model, df_ml.to.models[missing,1:13])

# data frame with predicted LFQ values by random forest model
imputed_kNN <- df_ml
imputed_kNN[missing,15] <- predictions2
...
```{r our way of imputation}
imputed_shift.rf <- imputation(rf.model, df_ml.to.models, df_ml)
```

```{r nonimp TOTALS data}
temp <- apply(lfq, 1, function(row) length(which(is.na(row))))
rows.to.omit <- which(temp > 2)

lfq <- lfq[-rows.to.omit,]
...
```{r}
# Random Forest
imputed_RF <- imputed_RF[-rows.to.omit,]
...
# K-Nearest Neighbors
imputed_kNN <- imputed_kNN[-rows.to.omit,]
...
# Ludovic
imputed_ludovic <- imputed_ludovic[-rows.to.omit,]

# extracting KO and WT
LFQ_KO_ludovic <- imputed_ludovic |> dplyr::select(contains('KO'))
LFQ_WT_ludovic <- imputed_ludovic |> dplyr::select(contains('WT'))

# Shift Dist + Random Forest
imputed_shift.rf <- imputed_shift.rf[-rows.to.omit,]

# extracting KO and WT
LFQ_KO_shift.rf <- imputed_shift.rf |> dplyr::select(contains('KO'))
LFQ_WT_shift.rf <- imputed_shift.rf |> dplyr::select(contains('WT'))
```
...
ggarrange(
    ...
    plothist(LFQ_KO_imp, '', FALSE) + xlab('KO') + ggtitle('Imputed - Shift Dist'), 
...
ggarrange(
    plothist(LFQ_KO, '', FALSE) + xlab('KO') + ggtitle('Non-Imputed'), 
    plothist(LFQ_WT, '', FALSE) + xlab('WT') + ggtitle(''),
    plothist(LFQ_KO_shift.rf, '', FALSE) + xlab('KO') + ggtitle('Imputed - Shift Dist + RF'), 
    plothist(LFQ_WT_shift.rf, '', FALSE) + xlab('WT') + ggtitle(''),
    nrow=2,ncol=2
)
```
...
skew_imp <- data.frame(raw_data = moments::skewness(2^lfq_imp,na.rm=T),
                   sqrt = moments::skewness(sqrt(2^lfq_imp),na.rm=T),
                   log = moments::skewness(log(2^lfq_imp),na.rm=T),
                   log10 = moments::skewness(log10(2^lfq_imp),na.rm=T),
                   `frac` = moments::skewness(1/(2^lfq_imp),na.rm=T))
...
skew_rf <- data.frame(raw_data = moments::skewness(2^imputed_RF,na.rm=T),
                   sqrt = moments::skewness(sqrt(2^imputed_RF),na.rm=T),
                   log = moments::skewness(log(2^imputed_RF),na.rm=T),
                   log10 = moments::skewness(log10(2^imputed_RF),na.rm=T),
                   `frac` = moments::skewness(1/(2^imputed_RF),na.rm=T))
...
skew_kNN <- data.frame(raw_data = moments::skewness(2^imputed_kNN,na.rm=T),
                   sqrt = moments::skewness(sqrt(2^imputed_kNN),na.rm=T),
                   log = moments::skewness(log(2^imputed_kNN),na.rm=T),
                   log10 = moments::skewness(log10(2^imputed_kNN),na.rm=T),
                   `frac` = moments::skewness(1/(2^imputed_kNN),na.rm=T))
...
```{r skew shift + RF}
skew_shift.rf <- data.frame(raw_data = moments::skewness(2^imputed_shift.rf[,1:6],na.rm=T),
                   sqrt = moments::skewness(sqrt(2^imputed_shift.rf[,1:6]),na.rm=T),
                   log = moments::skewness(log(2^imputed_shift.rf[,1:6]),na.rm=T),
                   log10 = moments::skewness(log10(2^imputed_shift.rf[,1:6]),na.rm=T),
                   `frac` = moments::skewness(1/(2^imputed_shift.rf[,1:6]),na.rm=T))
DT::datatable(t(round(skew_shift.rf,4)), options=list(paging=F, info=F, searching=F)) |>
    DT::formatStyle(3, backgroundColor ='lightgreen')
```
...
LFQ_KO_imp.standard <- as.data.frame(scale(LFQ_KO_imp)) # Imputed - Shift Dist
...
LFQ_KO_shift.rf.standard <- as.data.frame(scale(LFQ_KO_shift.rf)) # Imputed Shift Dist + RF
```

```{r WT standardization}
LFQ_WT.standard <- as.data.frame(scale(LFQ_WT)) # Non-Imputed
LFQ_WT_imp.standard <- as.data.frame(scale(LFQ_WT_imp)) # Imputed - Shift Dist
...
LFQ_WT_shift.rf.standard <- as.data.frame(scale(LFQ_WT_shift.rf)) # Imputed Shift Dist + RF
```
...
ggarrange(
    annotate_figure(ggarrange(
        plothist(LFQ_KO_imp.standard, NULL, FALSE) + xlab('KO'), 
        plothist(LFQ_WT_imp.standard, NULL, FALSE) + xlab('WT'),
        nrow=1,ncol=2
        ), top = text_grob('Imputed Data - Shift Dist', size = 16)
    ),
    annotate_figure(ggarrange(
        plothist(LFQ_KO_RF.standard, NULL, FALSE) + xlab('KO'), 
        plothist(LFQ_WT_RF.standard, NULL, FALSE) + xlab('WT'),
        nrow=1,ncol=2
...
ggarrange(
    annotate_figure(ggarrange(
        plothist(LFQ_KO_kNN.standard, NULL, FALSE) + xlab('KO'), 
        plothist(LFQ_WT_kNN.standard, NULL, FALSE) + xlab('WT'),
        nrow=1,ncol=2
        ), top = text_grob('Imputed Data - kNN', size = 16)
    ),
    annotate_figure(ggarrange(
        plothist(LFQ_KO_ludovic.standard, NULL, FALSE) + xlab('KO'), 
        plothist(LFQ_WT_ludovic.standard, NULL, FALSE) + xlab('WT'),
        nrow=1,ncol=2
...
```{r fig.height=4}
annotate_figure(ggarrange(
    plothist(LFQ_KO_shift.rf.standard, NULL, FALSE) + xlab('KO'), 
    plothist(LFQ_WT_shift.rf.standard, NULL, FALSE) + xlab('WT'),
    nrow=1,ncol=2
    ), top = text_grob('Imputed Data - Shift Dist + RF', size = 16)
)
```
...
LFQ_KO_imp.minmax <- as.data.frame(lapply(LFQ_KO_imp, function(col) min_max_norm(col)))  # Shift Dist
...
LFQ_KO_shift.rf.minmax <- as.data.frame(lapply(LFQ_KO_shift.rf, function(col) min_max_norm(col)))  # Imputed Shift Dist + RF

LFQ_WT.minmax <- as.data.frame(lapply(LFQ_WT, function(col) min_max_norm(col)))  # Non-Imputed
LFQ_WT_imp.minmax <- as.data.frame(lapply(LFQ_WT_imp, function(col) min_max_norm(col)))  # Shift Dist
...
LFQ_WT_shift.rf.minmax <- as.data.frame(lapply(LFQ_WT_shift.rf, function(col) min_max_norm(col)))  # Imputed Shift Dist + RF
```
...
ggarrange(
    annotate_figure(ggarrange(
        plothist(LFQ_KO_imp.minmax, NULL, FALSE) + xlab('KO'), 
        plothist(LFQ_WT_imp.minmax, NULL, FALSE) + xlab('WT'),
        nrow=1,ncol=2
        ), top = text_grob('Imputed Data - Shift Dist', size = 16)
    ),
    annotate_figure(ggarrange(
        plothist(LFQ_KO_RF.minmax, NULL, FALSE) + xlab('KO'), 
        plothist(LFQ_WT_RF.minmax, NULL, FALSE) + xlab('WT'),
        nrow=1,ncol=2
        ), top = text_grob('Imputed Data - Random Forest', size = 16)
    ),
    nrow=2,ncol=1
...
ggarrange(
    annotate_figure(ggarrange(
        plothist(LFQ_KO_kNN.minmax, NULL, FALSE) + xlab('KO'), 
        plothist(LFQ_WT_kNN.minmax, NULL, FALSE) + xlab('WT'),
        nrow=1,ncol=2
        ), top = text_grob('Imputed Data - kNN', size = 16)
    ),
    annotate_figure(ggarrange(
        plothist(LFQ_KO_ludovic.minmax, NULL, FALSE) + xlab('KO'), 
        plothist(LFQ_WT_ludovic.minmax, NULL, FALSE) + xlab('WT'),
        nrow=1,ncol=2
        ), top = text_grob('Imputed Data - Ludovic', size = 16)
    ),
    nrow=2,ncol=1
...
```{r fig.height=4}
annotate_figure(ggarrange(
    plothist(LFQ_KO.minmax, NULL, FALSE) + xlab('KO'), 
    plothist(LFQ_WT.minmax, NULL, FALSE) + xlab('WT'),
    nrow=1,ncol=2
    ), top = text_grob('Imputed Data - Shift Dist + RF', size = 16)
)
```
...
LFQ_KO_imp.med <- as.data.frame(DescTools::RobScale(LFQ_KO_imp, scale=FALSE)) # Imputed Shift Dist
LFQ_KO_RF.med <- as.data.frame(DescTools::RobScale(LFQ_KO_RF, scale=FALSE)) # Imputed RF
LFQ_KO_kNN.med <- as.data.frame(DescTools::RobScale(LFQ_KO_kNN, scale=FALSE)) # Imputed kNN
LFQ_KO_ludovic.med <- as.data.frame(DescTools::RobScale(LFQ_KO_ludovic, scale=FALSE)) # Imputed Ludovic
LFQ_KO_shift.rf.med <- as.data.frame(DescTools::RobScale(LFQ_KO_shift.rf, scale=FALSE)) # Imputed Shift Dist + RF

LFQ_WT.med <- as.data.frame(DescTools::RobScale(LFQ_WT, scale=FALSE)) # Non-Imputed
LFQ_WT_imp.med <- as.data.frame(DescTools::RobScale(LFQ_WT_imp, scale=FALSE)) # Imputed Shift Dist
LFQ_WT_RF.med <- as.data.frame(DescTools::RobScale(LFQ_WT_RF, scale=FALSE)) # Imputed RF
LFQ_WT_kNN.med <- as.data.frame(DescTools::RobScale(LFQ_WT_kNN, scale=FALSE)) # Imputed kNN
LFQ_WT_ludovic.med <- as.data.frame(DescTools::RobScale(LFQ_WT_ludovic, scale=FALSE)) # Imputed Ludovic
LFQ_WT_shift.rf.med <- as.data.frame(DescTools::RobScale(LFQ_WT_shift.rf, scale=FALSE)) # Imputed Shift Dist + RF
```
...
ggarrange(
    annotate_figure(ggarrange(
        plothist(LFQ_KO_imp.med, NULL, FALSE) + xlab('KO'), 
        plothist(LFQ_WT_imp.med, NULL, FALSE) + xlab('WT'),
        nrow=1,ncol=2
        ), top = text_grob('Imputed Data - Mati Code', size = 16)
    ),
    annotate_figure(ggarrange(
        plothist(LFQ_KO_RF.med, NULL, FALSE) + xlab('KO'), 
        plothist(LFQ_WT_RF.med, NULL, FALSE) + xlab('WT'),
        nrow=1,ncol=2
        ), top = text_grob('Imputed Data - Random Forest', size = 16)
    ),
    nrow=2,ncol=1
...
ggarrange(
    annotate_figure(ggarrange(
        plothist(LFQ_KO_kNN.med, NULL, FALSE) + xlab('KO'), 
        plothist(LFQ_WT_kNN.med, NULL, FALSE) + xlab('WT'),
        nrow=1,ncol=2
        ), top = text_grob('Imputed Data - kNN', size = 16)
    ),
    annotate_figure(ggarrange(
        plothist(LFQ_KO_ludovic.med, NULL, FALSE) + xlab('KO'), 
        plothist(LFQ_WT_ludovic.med, NULL, FALSE) + xlab('WT'),
        nrow=1,ncol=2
        ), top = text_grob('Imputed Data - Ludovic', size = 16)
    ),
    nrow=2,ncol=1
...
```{r fig.height=4}
annotate_figure(ggarrange(
    plothist(LFQ_KO_shift.rf.med, NULL, FALSE) + xlab('KO'), 
    plothist(LFQ_WT_shift.rf.med, NULL, FALSE) + xlab('WT'),
    nrow=1,ncol=2
    ), top = text_grob('Imputed Data - Shift Dist + RF', size = 16)
)
```
...
LFQ_KO_imp.mad <- as.data.frame(DescTools::RobScale(LFQ_KO_imp)) # Imputed Shift Dist
...
LFQ_KO_shift.rf.mad <- as.data.frame(DescTools::RobScale(LFQ_KO_shift.rf)) # Imputed Shift Dist + RF

LFQ_WT.mad <- as.data.frame(DescTools::RobScale(LFQ_WT)) # Non-Imputed
LFQ_WT_imp.mad <- as.data.frame(DescTools::RobScale(LFQ_WT_imp)) # Imputed Shift Dist
...
LFQ_WT_shift.rf.mad <- as.data.frame(DescTools::RobScale(LFQ_WT_shift.rf)) # Imputed Shift Dist + RF
```
...
ggarrange(
    annotate_figure(ggarrange(
        plothist(LFQ_KO_imp.mad, NULL, FALSE) + xlab('KO'), 
        plothist(LFQ_WT_imp.mad, NULL, FALSE) + xlab('WT'),
        nrow=1,ncol=2
        ), top = text_grob('Imputed Data - Mati Code', size = 16)
    ),
    annotate_figure(ggarrange(
        plothist(LFQ_KO_RF.mad, NULL, FALSE) + xlab('KO'), 
        plothist(LFQ_WT_RF.mad, NULL, FALSE) + xlab('WT'),
        nrow=1,ncol=2
        ), top = text_grob('Imputed Data - Random Forest', size = 16)
    ),
    nrow=2,ncol=1
...
ggarrange(
    annotate_figure(ggarrange(
        plothist(LFQ_KO_kNN.mad, NULL, FALSE) + xlab('KO'), 
        plothist(LFQ_WT_kNN.mad, NULL, FALSE) + xlab('WT'),
        nrow=1,ncol=2
        ), top = text_grob('Imputed Data - kNN', size = 16)
    ),
    annotate_figure(ggarrange(
        plothist(LFQ_KO_ludovic.mad, NULL, FALSE) + xlab('KO'), 
        plothist(LFQ_WT_ludovic.mad, NULL, FALSE) + xlab('WT'),
        nrow=1,ncol=2
        ), top = text_grob('Imputed Data - Ludovic', size = 16)
    ),
    nrow=2,ncol=1
...
```{r fig.height=4}
annotate_figure(ggarrange(
    plothist(LFQ_KO_shift.rf.mad, NULL, FALSE) + xlab('KO'), 
    plothist(LFQ_WT_shift.rf.mad, NULL, FALSE) + xlab('WT'),
    nrow=1,ncol=2
    ), top = text_grob('Imputed Data - Shift Dist + RF', size = 16)
)
```
...
# Imputed Shift Dist + RF
LFQ_KO_long <- LFQ_KO_shift.rf |>
    pivot_longer(cols = 1:3, names_to = "Batch", values_to = "LFQValue")
...
# reshape data to the wide format
LFQ_KO_shift.rf.lm <- LFQ_KO_long |> 
    mutate(row = rep(1:nrow(LFQ_KO_ludovic), each=3)) |>
    reshape2::dcast(row ~ Batch, value.var='normalized') |>
    select(-row)
```
...
# reshape data to the wide format
LFQ_WT_shift.rf.lm <- LFQ_WT_long |> 
    mutate(row = rep(1:nrow(LFQ_WT_ludovic), each=3)) |>
    reshape2::dcast(row ~ Batch, value.var='normalized') |>
    select(-row)
```
...
ggarrange(
    annotate_figure(ggarrange(
        plothist(LFQ_KO_imp.lm, NULL, FALSE) + xlab('KO'), 
        plothist(LFQ_WT_imp.lm, NULL, FALSE) + xlab('WT'),
        nrow=1,ncol=2
        ), top = text_grob('Imputed Data - Mati Code', size = 16)
    ),
    annotate_figure(ggarrange(
        plothist(LFQ_KO_RF.lm, NULL, FALSE) + xlab('KO'), 
        plothist(LFQ_WT_RF.lm, NULL, FALSE) + xlab('WT'),
        nrow=1,ncol=2
        ), top = text_grob('Imputed Data - Random Forest', size = 16)
    ),
    nrow=2,ncol=1
...
ggarrange(
    annotate_figure(ggarrange(
        plothist(LFQ_KO_kNN.lm, NULL, FALSE) + xlab('KO'), 
        plothist(LFQ_WT_kNN.lm, NULL, FALSE) + xlab('WT'),
        nrow=1,ncol=2
        ), top = text_grob('Imputed Data - kNN', size = 16)
    ),
    annotate_figure(ggarrange(
        plothist(LFQ_KO_ludovic.lm, NULL, FALSE) + xlab('KO'), 
        plothist(LFQ_WT_ludovic.lm, NULL, FALSE) + xlab('WT'),
        nrow=1,ncol=2
        ), top = text_grob('Imputed Data - Ludovic', size = 16)
    ),
    nrow=2,ncol=1
...
```{r fig.height=4}
annotate_figure(ggarrange(
    plothist(LFQ_KO_shift.rf.lm, NULL, FALSE) + xlab('KO'), 
    plothist(LFQ_WT_shift.rf.lm, NULL, FALSE) + xlab('WT'),
    nrow=1,ncol=2
    ), top = text_grob('Imputed Data - Shift Dist + RF', size = 16)
)
```
...
lfq_rglist_shift.rf <- new('RGList', list(
    R = as.matrix(2^LFQ_KO_shift.rf),   # condition
    G = as.matrix(2^LFQ_WT_shift.rf)))  # control
...
LFQ_KO_shift.rf.vsn <- justvsn(lfq_rglist_shift.rf)@assayData$R # Imputed Shift Dist + RF
...
LFQ_WT_shift.rf.vsn <- justvsn(lfq_rglist_shift.rf)@assayData$G # Imputed Shift Dist + RF
```
...
ggarrange(
    annotate_figure(ggarrange(
        plothist(LFQ_KO_imp.vsn, NULL, FALSE) + xlab('KO'), 
        plothist(LFQ_WT_imp.vsn, NULL, FALSE) + xlab('WT'),
        nrow=1,ncol=2
        ), top = text_grob('Imputed Data - Mati Code', size = 16)
    ),
    annotate_figure(ggarrange(
        plothist(LFQ_KO_RF.vsn, NULL, FALSE) + xlab('KO'), 
        plothist(LFQ_WT_RF.vsn, NULL, FALSE) + xlab('WT'),
        nrow=1,ncol=2
        ), top = text_grob('Imputed Data - Random Forest', size = 16)
    ),
    nrow=2,ncol=1
...
ggarrange(
    annotate_figure(ggarrange(
        plothist(LFQ_KO_kNN.vsn, NULL, FALSE) + xlab('KO'), 
        plothist(LFQ_WT_kNN.vsn, NULL, FALSE) + xlab('WT'),
        nrow=1,ncol=2
        ), top = text_grob('Imputed Data - kNN', size = 16)
    ),
    annotate_figure(ggarrange(
        plothist(LFQ_KO_ludovic.vsn, NULL, FALSE) + xlab('KO'), 
        plothist(LFQ_WT_ludovic.vsn, NULL, FALSE) + xlab('WT'),
        nrow=1,ncol=2
        ), top = text_grob('Imputed Data - Ludovic', size = 16)
    ),
    nrow=2,ncol=1
...
```{r fig.height=4}
annotate_figure(ggarrange(
    plothist(LFQ_KO_shift.rf.vsn, NULL, FALSE) + xlab('KO'), 
    plothist(LFQ_WT_shift.rf.vsn, NULL, FALSE) + xlab('WT'),
    nrow=1,ncol=2
    ), top = text_grob('Imputed Data - Shift Dist + RF', size = 16)
)
```
...
# Imputed Shift Dist + RF
prot.info <- data.frame(prot_ID = paste('prot_', 1:nrow(imputed_shift.rf), sep = '')) # add info about LFQ value
LFQ_shift.rf.eig1 <- eig_norm1(imputed_shift.rf[,1:6], 
                      treatment = treatment, 
                      prot.info = prot.info)
# Performing eig normalization
LFQ_shift.rf.eig_norm <- eig_norm2(LFQ_shift.rf.eig1)
```

```{r KO and WT eigen}
LFQ_KO.eigen <- LFQ.eig_norm$norm_m[,1:3] # Non-Imputed
LFQ_KO_imp.eigen <- LFQ_imp.eig_norm$norm_m[,1:3] # Imputed Shift Dist
...
LFQ_KO_shift.rf.eigen <- LFQ_shift.rf.eig_norm$norm_m[,1:3] # Imputed Shift Dist + RF

LFQ_WT.eigen <- LFQ.eig_norm$norm_m[,4:6] # Non-Imputed
LFQ_WT_imp.eigen <- LFQ_imp.eig_norm$norm_m[,4:6] # Imputed Shift Dist
...
LFQ_WT_shift.rf.eigen <- LFQ_shift.rf.eig_norm$norm_m[,4:6] # Imputed Shift Dist
```
...
```{r fig.height=4}
annotate_figure(ggarrange(
    plothist(LFQ_KO_shift.rf.eigen, NULL, FALSE) + xlab('KO'), 
    plothist(LFQ_WT_shift.rf.eigen, NULL, FALSE) + xlab('WT'),
    nrow=1,ncol=2
    ), top = text_grob('Non-Imputed Data', size = 16)
)
```
...
In this section we have interactive plots with additional information about the 
distribution of samples. Below you can see the tabs corresponding to the cell 
type (KO or WT) and the number of replicates.

## LFQ KO rep. 22

```{r prepare data ko 22}
# Prepare the data frame to plotting - Raw Data
to_plot_KO22.0 <- data.frame(
    method = 'Raw',
    original = LFQ_KO$KO.22,
    shift_dist = LFQ_KO_imp$KO.I.22,
    rf = LFQ_KO_RF$KO.22,
    knn = LFQ_KO_kNN$KO.22,
    ludovic = LFQ_KO_ludovic$KO_22,
    shift.rf = LFQ_KO_shift.rf$KO.22
)

# Prepare the data frame to plotting - Z-Score
to_plot_KO22.1 <- data.frame(
    method = 'Z-Score',
    original = LFQ_KO.standard$KO.22,
    shift_dist = LFQ_KO_imp.standard$KO.I.22,
    rf = LFQ_KO_RF.standard$KO.22,
    knn = LFQ_KO_kNN.standard$KO.22,
    ludovic = LFQ_KO_ludovic.standard$KO_22,
    shift.rf = LFQ_KO_shift.rf.standard$KO.22
)

# Prepare the data frame to plotting - Min-Max
to_plot_KO22.2 <- data.frame(
    method = 'Min-Max',
    original = LFQ_KO.minmax$KO.22,
    shift_dist = LFQ_KO_imp.minmax$KO.I.22,
    rf = LFQ_KO_RF.minmax$KO.22,
    knn = LFQ_KO_kNN.minmax$KO.22,
    ludovic = LFQ_KO_ludovic.minmax$KO_22,
    shift.rf = LFQ_KO_shift.rf.minmax$KO.22
)

# Prepare the data frame to plotting - Median Scaling
to_plot_KO22.3 <- data.frame(
    method = 'Median',
    original = LFQ_KO.med$KO.22,
    shift_dist = LFQ_KO_imp.med$KO.I.22,
    rf = LFQ_KO_RF.med$KO.22,
    knn = LFQ_KO_kNN.med$KO.22,
    ludovic = LFQ_KO_ludovic.med$KO_22,
    shift.rf = LFQ_KO_shift.rf.med$KO.22
)

# Prepare the data frame to plotting - MAD Scaling
to_plot_KO22.4 <- data.frame(
    method = 'MAD',
    original = LFQ_KO.mad$KO.22,
    shift_dist = LFQ_KO_imp.mad$KO.I.22,
    rf = LFQ_KO_RF.mad$KO.22,
    knn = LFQ_KO_kNN.mad$KO.22,
    ludovic = LFQ_KO_ludovic.mad$KO_22,
    shift.rf = LFQ_KO_shift.rf.mad$KO.22
)

# Prepare the data frame to plotting - Linear Regression
to_plot_KO22.5 <- data.frame(
    method = 'Linear Regression',
    original = LFQ_KO.lm$KO.22,
    shift_dist = LFQ_KO_imp.lm$KO.I.22,
    rf = LFQ_KO_RF.lm$KO.22,
    knn = LFQ_KO_kNN.lm$KO.22,
    ludovic = LFQ_KO_ludovic.lm$KO_22,
    shift.rf = LFQ_KO_shift.rf.lm$KO.22
)

# Prepare the data frame to plotting - VSN
to_plot_KO22.6 <- data.frame(
    method = 'VSN',
    original = LFQ_KO.vsn[,1],
    shift_dist = LFQ_KO_imp.vsn[,1],
    rf = LFQ_KO_RF.vsn[,1],
    knn = LFQ_KO_kNN.vsn[,1],
    ludovic = LFQ_KO_ludovic.vsn[,1],
    shift.rf = LFQ_KO_shift.rf.vsn[,1]
)

# Prepare the data frame to plotting - EigenMS
to_plot_KO22.7 <- data.frame(
    method = 'EigenMS',
    original = LFQ_KO.eigen[,1],
    shift_dist = LFQ_KO_imp.eigen[,1],
    rf = LFQ_KO_RF.eigen[,1],
    knn = LFQ_KO_kNN.eigen[,1],
    ludovic = LFQ_KO_ludovic.eigen[,1],
    shift.rf = LFQ_KO_shift.rf.eigen[,1]
)

to_plot_KO22 <- rbind.data.frame(to_plot_KO22.0, to_plot_KO22.1, to_plot_KO22.2, 
                                 to_plot_KO22.3, to_plot_KO22.4, to_plot_KO22.5, 
                                 to_plot_KO22.6, to_plot_KO22.7)
rm(to_plot_KO22.0, to_plot_KO22.1, to_plot_KO22.2, to_plot_KO22.3, to_plot_KO22.4,
   to_plot_KO22.5, to_plot_KO22.6, to_plot_KO22.7)
```

```{r plotting ko 22}
# Define user input from a list of normalization methods
shiny::selectInput('method1', 'Select the Normalization Method:', unique(to_plot_KO22$method))

# Creating output with interactive histogram
output$lfq.ko.22.Plot <- renderPlotly({
    selected_method1 <- input$method1

    # Filter data for selected gene
    filtered_data <- to_plot_KO22 |>
        dplyr::filter(method == selected_method1) |>
        tidyr::pivot_longer(2:7, names_to='Type', values_to = 'LFQ_KO_22')
    
    plotly::plot_ly(data = filtered_data, x = ~LFQ_KO_22, split = ~Type,
                    type = 'histogram', opacity = 0.5, histfunc = 'count', xbins = 20) |>
        plotly::layout(
            title = paste('Distributions of the Normalized Data using', selected_method1),
            xaxis = list(title='LFQ KO rep. 22'),
...
```{r}
plotlyOutput("lfq.ko.22.Plot")
```

## LFQ KO rep. 23

```{r prepare data ko 23}
# extract the IDs of protein imputed by ludovic method
ludovic_ids <- as.numeric(gsub('prot_', '', imputed_ludovic$`Protein IDs`))

# Prepare the data frame to plotting - Raw Data
to_plot_KO23.0 <- data.frame(
    method = 'Raw',
    original = LFQ_KO$KO.23,
    shift_dist = LFQ_KO_imp$KO.I.23,
    rf = LFQ_KO_RF$KO.23,
    knn = LFQ_KO_kNN$KO.23,
    ludovic = NA
)
to_plot_KO23.0$ludovic[ludovic_ids] <- LFQ_KO_ludovic$KO_23

# Prepare the data frame to plotting - Z-Score
to_plot_KO23.1 <- data.frame(
    method = 'Z-Score',
    original = LFQ_KO.standard$KO.23,
    shift_dist = LFQ_KO_imp.standard$KO.I.23,
    rf = LFQ_KO_RF.standard$KO.23,
    knn = LFQ_KO_kNN.standard$KO.23,
    ludovic = NA
)
to_plot_KO23.1$ludovic[ludovic_ids] <- LFQ_KO_ludovic.standard$KO_23

# Prepare the data frame to plotting - Min-Max
to_plot_KO23.2 <- data.frame(
    method = 'Min-Max',
    original = LFQ_KO.minmax$KO.23,
    shift_dist = LFQ_KO_imp.minmax$KO.I.23,
    rf = LFQ_KO_RF.minmax$KO.23,
    knn = LFQ_KO_kNN.minmax$KO.23,
    ludovic = NA
)
to_plot_KO23.2$ludovic[ludovic_ids] <- LFQ_KO_ludovic.minmax$KO_23

# Prepare the data frame to plotting - Median Scaling
to_plot_KO23.3 <- data.frame(
    method = 'Median',
    original = LFQ_KO.med$KO.23,
    shift_dist = LFQ_KO_imp.med$KO.I.23,
    rf = LFQ_KO_RF.med$KO.23,
    knn = LFQ_KO_kNN.med$KO.23,
    ludovic = NA
)
to_plot_KO23.3$ludovic[ludovic_ids] <- LFQ_KO_ludovic.med$KO_23

# Prepare the data frame to plotting - MAD Scaling
to_plot_KO23.4 <- data.frame(
    method = 'MAD',
    original = LFQ_KO.mad$KO.23,
    shift_dist = LFQ_KO_imp.mad$KO.I.23,
    rf = LFQ_KO_RF.mad$KO.23,
    knn = LFQ_KO_kNN.mad$KO.23,
    ludovic = NA
)
to_plot_KO23.4$ludovic[ludovic_ids] <- LFQ_KO_ludovic.mad$KO_23

# Prepare the data frame to plotting - Linear Regression
to_plot_KO23.5 <- data.frame(
    method = 'Linear Regression',
    original = LFQ_KO.lm$KO.23,
    shift_dist = LFQ_KO_imp.lm$KO.I.23,
    rf = LFQ_KO_RF.lm$KO.23,
    knn = LFQ_KO_kNN.lm$KO.23,
    ludovic = NA
)
to_plot_KO23.5$ludovic[ludovic_ids] <- LFQ_KO_ludovic.lm$KO_23

# Prepare the data frame to plotting - VSN
to_plot_KO23.6 <- data.frame(
    method = 'VSN',
    original = LFQ_KO.vsn[,2],
    shift_dist = LFQ_KO_imp.vsn[,2],
    rf = LFQ_KO_RF.vsn[,2],
    knn = LFQ_KO_kNN.vsn[,2],
    ludovic = NA
)
to_plot_KO23.6$ludovic[ludovic_ids] <- LFQ_KO_ludovic.vsn[,2]

raw_ids <- as.numeric(gsub('prot_', '', rownames(LFQ_KO.eigen)))
# Prepare the data frame to plotting - EigenMS
to_plot_KO23.7 <- data.frame(
    method = 'EigenMS',
    original = NA,
    shift_dist = LFQ_KO_imp.eigen[,2],
    rf = LFQ_KO_RF.eigen[,2],
    knn = LFQ_KO_kNN.eigen[,2],
    ludovic = NA
)
to_plot_KO23.7$original[raw_ids] <- LFQ_KO.eigen[,2]
to_plot_KO23.7$ludovic[ludovic_ids] <- LFQ_KO_ludovic.eigen[,2]

to_plot_KO23 <- rbind.data.frame(to_plot_KO23.0, to_plot_KO23.1, to_plot_KO23.2, 
                                 to_plot_KO23.3, to_plot_KO23.4, to_plot_KO23.5, 
                                 to_plot_KO23.6, to_plot_KO23.7)
rm(to_plot_KO23.0, to_plot_KO23.1, to_plot_KO23.2, to_plot_KO23.3, to_plot_KO23.4,
   to_plot_KO23.5, to_plot_KO23.6, to_plot_KO23.7, ludovic_ids, raw_ids)
```

```{r plotting ko 23}
# Define user input from a list of normalization methods
shiny::selectInput('method2', 'Select the Normalization Method:', unique(to_plot_KO23$method))

# Creating output with interactive histogram
output$lfq.ko.23.Plot <- renderPlotly({
    selected_method2 <- input$method2
    
    # Filter data for selected gene
    filtered_data <- to_plot_KO23 |>
        dplyr::filter(method == selected_method2) |>
        tidyr::pivot_longer(2:6, names_to='Type', values_to = 'LFQ_KO_23')
    
    plotly::plot_ly(data = filtered_data, x = ~LFQ_KO_23, split = ~Type,
                    type = 'histogram', opacity = 0.5, histfunc = 'count', xbins = 20) |>
        plotly::layout(
            title = paste('Distributions of the Normalized Data using', selected_method2),
            xaxis = list(title='LFQ KO rep. 23'),
            yaxis = list(title='Frequency'),
            barmode = 'overlay'
        )
})
...
```{r}
plotlyOutput("lfq.ko.23.Plot")
```

## LFQ KO rep. 24

```{r prepare data ko 24}
# extract the IDs of protein imputed by ludovic method
ludovic_ids <- as.numeric(gsub('prot_', '', imputed_ludovic$`Protein IDs`))

# Prepare the data frame to plotting - Raw Data
to_plot_KO24.0 <- data.frame(
    method = 'Raw',
    original = LFQ_KO$KO.24,
    shift_dist = LFQ_KO_imp$KO.I.24,
    rf = LFQ_KO_RF$KO.24,
    knn = LFQ_KO_kNN$KO.24,
    ludovic = NA
)
to_plot_KO24.0$ludovic[ludovic_ids] <- LFQ_KO_ludovic$KO_24

# Prepare the data frame to plotting - Z-Score
to_plot_KO24.1 <- data.frame(
    method = 'Z-Score',
    original = LFQ_KO.standard$KO.24,
    shift_dist = LFQ_KO_imp.standard$KO.I.24,
    rf = LFQ_KO_RF.standard$KO.24,
    knn = LFQ_KO_kNN.standard$KO.24,
    ludovic = NA
)
to_plot_KO24.1$ludovic[ludovic_ids] <- LFQ_KO_ludovic.standard$KO_24

# Prepare the data frame to plotting - Min-Max
to_plot_KO24.2 <- data.frame(
    method = 'Min-Max',
    original = LFQ_KO.minmax$KO.24,
    shift_dist = LFQ_KO_imp.minmax$KO.I.24,
    rf = LFQ_KO_RF.minmax$KO.24,
    knn = LFQ_KO_kNN.minmax$KO.24,
    ludovic = NA
)
to_plot_KO24.2$ludovic[ludovic_ids] <- LFQ_KO_ludovic.minmax$KO_24

# Prepare the data frame to plotting - Median Scaling
to_plot_KO24.3 <- data.frame(
    method = 'Median',
    original = LFQ_KO.med$KO.24,
    shift_dist = LFQ_KO_imp.med$KO.I.24,
    rf = LFQ_KO_RF.med$KO.24,
    knn = LFQ_KO_kNN.med$KO.24,
    ludovic = NA
)
to_plot_KO24.3$ludovic[ludovic_ids] <- LFQ_KO_ludovic.med$KO_24

# Prepare the data frame to plotting - MAD Scaling
to_plot_KO24.4 <- data.frame(
    method = 'MAD',
    original = LFQ_KO.mad$KO.24,
    shift_dist = LFQ_KO_imp.mad$KO.I.24,
    rf = LFQ_KO_RF.mad$KO.24,
    knn = LFQ_KO_kNN.mad$KO.24,
    ludovic = NA
)
to_plot_KO24.4$ludovic[ludovic_ids] <- LFQ_KO_ludovic.mad$KO_24

# Prepare the data frame to plotting - Linear Regression
to_plot_KO24.5 <- data.frame(
    method = 'Linear Regression',
    original = LFQ_KO.lm$KO.24,
    shift_dist = LFQ_KO_imp.lm$KO.I.24,
    rf = LFQ_KO_RF.lm$KO.24,
    knn = LFQ_KO_kNN.lm$KO.24,
    ludovic = NA
)
to_plot_KO24.5$ludovic[ludovic_ids] <- LFQ_KO_ludovic.lm$KO_24

# Prepare the data frame to plotting - VSN
to_plot_KO24.6 <- data.frame(
    method = 'VSN',
    original = LFQ_KO.vsn[,3],
    shift_dist = LFQ_KO_imp.vsn[,3],
    rf = LFQ_KO_RF.vsn[,3],
    knn = LFQ_KO_kNN.vsn[,3],
    ludovic = NA
)
to_plot_KO24.6$ludovic[ludovic_ids] <- LFQ_KO_ludovic.vsn[,3]

raw_ids <- as.numeric(gsub('prot_', '', rownames(LFQ_KO.eigen)))
# Prepare the data frame to plotting - EigenMS
to_plot_KO24.7 <- data.frame(
    method = 'EigenMS',
    original = NA,
    shift_dist = LFQ_KO_imp.eigen[,3],
    rf = LFQ_KO_RF.eigen[,3],
    knn = LFQ_KO_kNN.eigen[,3],
    ludovic = NA
)
to_plot_KO24.7$original[raw_ids] <- LFQ_KO.eigen[,3]
to_plot_KO24.7$ludovic[ludovic_ids] <- LFQ_KO_ludovic.eigen[,3]

to_plot_KO24 <- rbind.data.frame(to_plot_KO24.0, to_plot_KO24.1, to_plot_KO24.2, 
                                 to_plot_KO24.3, to_plot_KO24.4, to_plot_KO24.5, 
                                 to_plot_KO24.6, to_plot_KO24.7)
rm(to_plot_KO24.0, to_plot_KO24.1, to_plot_KO24.2, to_plot_KO24.3, to_plot_KO24.4, 
   to_plot_KO24.5, to_plot_KO24.6, to_plot_KO24.7, ludovic_ids, raw_ids)
```

```{r plotting ko 24}
# Define user input from a list of normalization methods
shiny::selectInput('method3', 'Select the Normalization Method:', unique(to_plot_KO24$method))

# Creating output with interactive histogram
output$lfq.ko.24.Plot <- renderPlotly({
    selected_method3 <- input$method3
    
    # Filter data for selected gene
    filtered_data <- to_plot_KO24 |>
        dplyr::filter(method == selected_method3) |>
        tidyr::pivot_longer(2:6, names_to='Type', values_to = 'LFQ_KO_24')
    
    plotly::plot_ly(data = filtered_data, x = ~LFQ_KO_24, split = ~Type,
                    type = 'histogram', opacity = 0.5, histfunc = 'count', xbins = 20) |>
        plotly::layout(
            title = paste('Distributions of the Normalized Data using', selected_method3),
            xaxis = list(title='LFQ KO rep. 24'),
            yaxis = list(title='Frequency'),
            barmode = 'overlay'
        )
})
...
```{r}
plotlyOutput("lfq.ko.24.Plot")
```

## LFQ WT rep. 22

```{r prepare data WT 22}
# extract the IDs of protein imputed by ludovic method
ludovic_ids <- as.numeric(gsub('prot_', '', imputed_ludovic$`Protein IDs`))

# Prepare the data frame to plotting - Raw Data
to_plot_WT22.0 <- data.frame(
    method = 'Raw',
    original = LFQ_WT$WT.22,
    shift_dist = LFQ_WT_imp$WT.I.22,
    rf = LFQ_WT_RF$WT.22,
    knn = LFQ_WT_kNN$WT.22,
    ludovic = NA
)
to_plot_WT22.0$ludovic[ludovic_ids] <- LFQ_WT_ludovic$WT_22

# Prepare the data frame to plotting - Z-Score
to_plot_WT22.1 <- data.frame(
    method = 'Z-Score',
    original = LFQ_WT.standard$WT.22,
    shift_dist = LFQ_WT_imp.standard$WT.I.22,
    rf = LFQ_WT_RF.standard$WT.22,
    knn = LFQ_WT_kNN.standard$WT.22,
    ludovic = NA
)
to_plot_WT22.1$ludovic[ludovic_ids] <- LFQ_WT_ludovic.standard$WT_22

# Prepare the data frame to plotting - Min-Max
to_plot_WT22.2 <- data.frame(
    method = 'Min-Max',
    original = LFQ_WT.minmax$WT.22,
    shift_dist = LFQ_WT_imp.minmax$WT.I.22,
    rf = LFQ_WT_RF.minmax$WT.22,
    knn = LFQ_WT_kNN.minmax$WT.22,
    ludovic = NA
)
to_plot_WT22.2$ludovic[ludovic_ids] <- LFQ_WT_ludovic.minmax$WT_22

# Prepare the data frame to plotting - Median Scaling
to_plot_WT22.3 <- data.frame(
    method = 'Median',
    original = LFQ_WT.med$WT.22,
    shift_dist = LFQ_WT_imp.med$WT.I.22,
    rf = LFQ_WT_RF.med$WT.22,
    knn = LFQ_WT_kNN.med$WT.22,
    ludovic = NA
)
to_plot_WT22.3$ludovic[ludovic_ids] <- LFQ_WT_ludovic.med$WT_22

# Prepare the data frame to plotting - MAD Scaling
to_plot_WT22.4 <- data.frame(
    method = 'MAD',
    original = LFQ_WT.mad$WT.22,
    shift_dist = LFQ_WT_imp.mad$WT.I.22,
    rf = LFQ_WT_RF.mad$WT.22,
    knn = LFQ_WT_kNN.mad$WT.22,
    ludovic = NA
)
to_plot_WT22.4$ludovic[ludovic_ids] <- LFQ_WT_ludovic.mad$WT_22

# Prepare the data frame to plotting - Linear Regression
to_plot_WT22.5 <- data.frame(
    method = 'Linear Regression',
    original = LFQ_WT.lm$WT.22,
    shift_dist = LFQ_WT_imp.lm$WT.I.22,
    rf = LFQ_WT_RF.lm$WT.22,
    knn = LFQ_WT_kNN.lm$WT.22,
    ludovic = NA
)
to_plot_WT22.5$ludovic[ludovic_ids] <- LFQ_WT_ludovic.lm$WT_22

# Prepare the data frame to plotting - VSN
to_plot_WT22.6 <- data.frame(
    method = 'VSN',
    original = LFQ_WT.vsn[,1],
    shift_dist = LFQ_WT_imp.vsn[,1],
    rf = LFQ_WT_RF.vsn[,1],
    knn = LFQ_WT_kNN.vsn[,1],
    ludovic = NA
)
to_plot_WT22.6$ludovic[ludovic_ids] <- LFQ_WT_ludovic.vsn[,1]

raw_ids <- as.numeric(gsub('prot_', '', rownames(LFQ_WT.eigen)))
# Prepare the data frame to plotting - EigenMS
to_plot_WT22.7 <- data.frame(
    method = 'EigenMS',
    original = NA,
    shift_dist = LFQ_WT_imp.eigen[,1],
    rf = LFQ_WT_RF.eigen[,1],
    knn = LFQ_WT_kNN.eigen[,1],
    ludovic = NA
)
to_plot_WT22.7$original[raw_ids] <- LFQ_WT.eigen[,1]
to_plot_WT22.7$ludovic[ludovic_ids] <- LFQ_WT_ludovic.eigen[,1]

to_plot_WT22 <- rbind.data.frame(to_plot_WT22.0, to_plot_WT22.1, to_plot_WT22.2, 
                                 to_plot_WT22.3, to_plot_WT22.4, to_plot_WT22.5, 
                                 to_plot_WT22.6, to_plot_WT22.7)
rm(to_plot_WT22.0, to_plot_WT22.1, to_plot_WT22.2, to_plot_WT22.3, to_plot_WT22.4, 
   to_plot_WT22.5, to_plot_WT22.6, to_plot_WT22.7, ludovic_ids, raw_ids)
```

```{r plotting WT 22}
# Define user input from a list of normalization methods
shiny::selectInput('method4', 'Select the Normalization Method:', unique(to_plot_WT22$method))

# Creating output with interactive histogram
output$lfq.WT.22.Plot <- renderPlotly({
    selected_method4 <- input$method4

    # Filter data for selected gene
    filtered_data <- to_plot_WT22 |>
        dplyr::filter(method == selected_method4) |>
        tidyr::pivot_longer(2:6, names_to='Type', values_to = 'LFQ_WT_22')
    
    plotly::plot_ly(data = filtered_data, x = ~LFQ_WT_22, split = ~Type,
                    type = 'histogram', opacity = 0.5, histfunc = 'count', xbins = 20) |>
        plotly::layout(
            title = paste('Distributions of the Normalized Data using', selected_method4),
            xaxis = list(title='LFQ WT rep. 22'),
            yaxis = list(title='Frequency'),
            barmode = 'overlay'
        )
})
...
```{r}
plotlyOutput("lfq.WT.22.Plot")
```

## LFQ WT rep. 23

```{r prepare data WT 23}
# extract the IDs of protein imputed by ludovic method
ludovic_ids <- as.numeric(gsub('prot_', '', imputed_ludovic$`Protein IDs`))

# Prepare the data frame to plotting - Raw Data
to_plot_WT23.0 <- data.frame(
    method = 'Raw',
    original = LFQ_WT$WT.23,
    shift_dist = LFQ_WT_imp$WT.I.23,
    rf = LFQ_WT_RF$WT.23,
    knn = LFQ_WT_kNN$WT.23,
    ludovic = NA
)
to_plot_WT23.0$ludovic[ludovic_ids] <- LFQ_WT_ludovic$WT_23

# Prepare the data frame to plotting - Z-Score
to_plot_WT23.1 <- data.frame(
    method = 'Z-Score',
    original = LFQ_WT.standard$WT.23,
    shift_dist = LFQ_WT_imp.standard$WT.I.23,
    rf = LFQ_WT_RF.standard$WT.23,
    knn = LFQ_WT_kNN.standard$WT.23,
    ludovic = NA
)
to_plot_WT23.1$ludovic[ludovic_ids] <- LFQ_WT_ludovic.standard$WT_23

# Prepare the data frame to plotting - Min-Max
to_plot_WT23.2 <- data.frame(
    method = 'Min-Max',
    original = LFQ_WT.minmax$WT.23,
    shift_dist = LFQ_WT_imp.minmax$WT.I.23,
    rf = LFQ_WT_RF.minmax$WT.23,
    knn = LFQ_WT_kNN.minmax$WT.23,
    ludovic = NA
)
to_plot_WT23.2$ludovic[ludovic_ids] <- LFQ_WT_ludovic.minmax$WT_23

# Prepare the data frame to plotting - Median Scaling
to_plot_WT23.3 <- data.frame(
    method = 'Median',
    original = LFQ_WT.med$WT.23,
    shift_dist = LFQ_WT_imp.med$WT.I.23,
    rf = LFQ_WT_RF.med$WT.23,
    knn = LFQ_WT_kNN.med$WT.23,
    ludovic = NA
)
to_plot_WT23.3$ludovic[ludovic_ids] <- LFQ_WT_ludovic.med$WT_23

# Prepare the data frame to plotting - MAD Scaling
to_plot_WT23.4 <- data.frame(
    method = 'MAD',
    original = LFQ_WT.mad$WT.23,
    shift_dist = LFQ_WT_imp.mad$WT.I.23,
    rf = LFQ_WT_RF.mad$WT.23,
    knn = LFQ_WT_kNN.mad$WT.23,
    ludovic = NA
)
to_plot_WT23.4$ludovic[ludovic_ids] <- LFQ_WT_ludovic.mad$WT_23

# Prepare the data frame to plotting - Linear Regression
to_plot_WT23.5 <- data.frame(
    method = 'Linear Regression',
    original = LFQ_WT.lm$WT.23,
    shift_dist = LFQ_WT_imp.lm$WT.I.23,
    rf = LFQ_WT_RF.lm$WT.23,
    knn = LFQ_WT_kNN.lm$WT.23,
    ludovic = NA
)
to_plot_WT23.5$ludovic[ludovic_ids] <- LFQ_WT_ludovic.lm$WT_23

# Prepare the data frame to plotting - VSN
to_plot_WT23.6 <- data.frame(
    method = 'VSN',
    original = LFQ_WT.vsn[,2],
    shift_dist = LFQ_WT_imp.vsn[,2],
    rf = LFQ_WT_RF.vsn[,2],
    knn = LFQ_WT_kNN.vsn[,2],
    ludovic = NA
)
to_plot_WT23.6$ludovic[ludovic_ids] <- LFQ_WT_ludovic.vsn[,2]

raw_ids <- as.numeric(gsub('prot_', '', rownames(LFQ_WT.eigen)))
# Prepare the data frame to plotting - EigenMS
to_plot_WT23.7 <- data.frame(
    method = 'EigenMS',
    original = NA,
    shift_dist = LFQ_WT_imp.eigen[,2],
    rf = LFQ_WT_RF.eigen[,2],
    knn = LFQ_WT_kNN.eigen[,2],
    ludovic = NA
)
to_plot_WT23.7$original[raw_ids] <- LFQ_WT.eigen[,2]
to_plot_WT23.7$ludovic[ludovic_ids] <- LFQ_WT_ludovic.eigen[,2]

to_plot_WT23 <- rbind.data.frame(to_plot_WT23.0, to_plot_WT23.1, to_plot_WT23.2, 
                                 to_plot_WT23.3, to_plot_WT23.4, to_plot_WT23.5, 
                                 to_plot_WT23.6, to_plot_WT23.7)
rm(to_plot_WT23.0, to_plot_WT23.1, to_plot_WT23.2, to_plot_WT23.3, to_plot_WT23.4, 
   to_plot_WT23.5, to_plot_WT23.6, to_plot_WT23.7, ludovic_ids, raw_ids)
```

```{r plotting WT 23}
# Define user input from a list of normalization methods
shiny::selectInput('method5', 'Select the Normalization Method:', unique(to_plot_WT23$method))

# Creating output with interactive histogram
output$lfq.WT.23.Plot <- renderPlotly({
    selected_method5 <- input$method5
    
    # Filter data for selected gene
    filtered_data <- to_plot_WT23 |>
        dplyr::filter(method == selected_method5) |>
        tidyr::pivot_longer(2:6, names_to='Type', values_to = 'LFQ_WT_23')
    
    plotly::plot_ly(data = filtered_data, x = ~LFQ_WT_23, split = ~Type,
                    type = 'histogram', opacity = 0.5, histfunc = 'count', xbins = 20) |>
        plotly::layout(
            title = paste('Distributions of the Normalized Data using', selected_method5),
            xaxis = list(title='LFQ WT rep. 23'),
            yaxis = list(title='Frequency'),
            barmode = 'overlay'
        )
})
...
```{r}
plotlyOutput("lfq.WT.23.Plot")
```

## LFQ WT rep. 24

```{r prepare data WT 24}
# extract the IDs of protein imputed by ludovic method
ludovic_ids <- as.numeric(gsub('prot_', '', imputed_ludovic$`Protein IDs`))

# Prepare the data frame to plotting - Raw Data
to_plot_WT24.0 <- data.frame(
    method = 'Raw',
    original = LFQ_WT$WT.24,
    shift_dist = LFQ_WT_imp$WT.I.24,
    rf = LFQ_WT_RF$WT.24,
    knn = LFQ_WT_kNN$WT.24,
    ludovic = NA
)
to_plot_WT24.0$ludovic[ludovic_ids] <- LFQ_WT_ludovic$WT_24

# Prepare the data frame to plotting - Z-Score
to_plot_WT24.1 <- data.frame(
    method = 'Z-Score',
    original = LFQ_WT.standard$WT.24,
    shift_dist = LFQ_WT_imp.standard$WT.I.24,
    rf = LFQ_WT_RF.standard$WT.24,
    knn = LFQ_WT_kNN.standard$WT.24,
    ludovic = NA
)
to_plot_WT24.1$ludovic[ludovic_ids] <- LFQ_WT_ludovic.standard$WT_24

# Prepare the data frame to plotting - Min-Max
to_plot_WT24.2 <- data.frame(
    method = 'Min-Max',
    original = LFQ_WT.minmax$WT.24,
    shift_dist = LFQ_WT_imp.minmax$WT.I.24,
    rf = LFQ_WT_RF.minmax$WT.24,
    knn = LFQ_WT_kNN.minmax$WT.24,
    ludovic = NA
)
to_plot_WT24.2$ludovic[ludovic_ids] <- LFQ_WT_ludovic.minmax$WT_24

# Prepare the data frame to plotting - Median Scaling
to_plot_WT24.3 <- data.frame(
    method = 'Median',
    original = LFQ_WT.med$WT.24,
    shift_dist = LFQ_WT_imp.med$WT.I.24,
    rf = LFQ_WT_RF.med$WT.24,
    knn = LFQ_WT_kNN.med$WT.24,
    ludovic = NA
)
to_plot_WT24.3$ludovic[ludovic_ids] <- LFQ_WT_ludovic.med$WT_24

# Prepare the data frame to plotting - MAD Scaling
to_plot_WT24.4 <- data.frame(
    method = 'MAD',
    original = LFQ_WT.mad$WT.24,
    shift_dist = LFQ_WT_imp.mad$WT.I.24,
    rf = LFQ_WT_RF.mad$WT.24,
    knn = LFQ_WT_kNN.mad$WT.24,
    ludovic = NA
)
to_plot_WT24.4$ludovic[ludovic_ids] <- LFQ_WT_ludovic.mad$WT_24

# Prepare the data frame to plotting - Linear Regression
to_plot_WT24.5 <- data.frame(
    method = 'Linear Regression',
    original = LFQ_WT.lm$WT.24,
    shift_dist = LFQ_WT_imp.lm$WT.I.24,
    rf = LFQ_WT_RF.lm$WT.24,
    knn = LFQ_WT_kNN.lm$WT.24,
    ludovic = NA
)
to_plot_WT24.5$ludovic[ludovic_ids] <- LFQ_WT_ludovic.lm$WT_24

# Prepare the data frame to plotting - VSN
to_plot_WT24.6 <- data.frame(
    method = 'VSN',
    original = LFQ_WT.vsn[,3],
    shift_dist = LFQ_WT_imp.vsn[,3],
    rf = LFQ_WT_RF.vsn[,3],
    knn = LFQ_WT_kNN.vsn[,3],
    ludovic = NA
)
to_plot_WT24.6$ludovic[ludovic_ids] <- LFQ_WT_ludovic.vsn[,3]

raw_ids <- as.numeric(gsub('prot_', '', rownames(LFQ_WT.eigen)))
# Prepare the data frame to plotting - EigenMS
to_plot_WT24.7 <- data.frame(
    method = 'EigenMS',
    original = NA,
    shift_dist = LFQ_WT_imp.eigen[,3],
    rf = LFQ_WT_RF.eigen[,3],
    knn = LFQ_WT_kNN.eigen[,3],
    ludovic = NA
)
to_plot_WT24.7$original[raw_ids] <- LFQ_WT.eigen[,3]
to_plot_WT24.7$ludovic[ludovic_ids] <- LFQ_WT_ludovic.eigen[,3]

to_plot_WT24 <- rbind.data.frame(to_plot_WT24.0, to_plot_WT24.1, to_plot_WT24.2, 
                                 to_plot_WT24.3, to_plot_WT24.4, to_plot_WT24.5, 
                                 to_plot_WT24.6, to_plot_WT24.7)
rm(to_plot_WT24.0, to_plot_WT24.1, to_plot_WT24.2, to_plot_WT24.3, to_plot_WT24.4, 
   to_plot_WT24.5, to_plot_WT24.6, to_plot_WT24.7, ludovic_ids, raw_ids)
```

```{r plotting WT 24}
# Define user input from a list of normalization methods
shiny::selectInput('method6', 'Select the Normalization Method:', unique(to_plot_WT24$method))

# Creating output with interactive histogram
output$lfq.WT.24.Plot <- renderPlotly({
    selected_method6 <- input$method6
    
    # Filter data for selected gene
    filtered_data <- to_plot_WT24 |>
        dplyr::filter(method == selected_method6) |>
        tidyr::pivot_longer(2:6, names_to='Type', values_to = 'LFQ_WT_24')
    
    plotly::plot_ly(data = filtered_data, x = ~LFQ_WT_24, split = ~Type,
                    type = 'histogram', opacity = 0.5, histfunc = 'count', xbins = 20) |>
        plotly::layout(
            title = paste('Distributions of the Normalized Data using', selected_method6),
            xaxis = list(title='LFQ WT rep. 24'),
            yaxis = list(title='Frequency'),
            barmode = 'overlay'
        )
})
...
```{r}
plotlyOutput("lfq.WT.24.Plot")
```

## All methods

```{r all dist}
to_plot_raw <- data.frame(
    original = lfq
)

to_plot_all <- rbind(to_plot_KO22,
                     to_plot_WT22,
                     to_plot_KO23,
                     to_plot_WT23,
                     to_plot_KO24,
                     to_plot_WT23)
to_plot_all$celltype <- rep(c('KO.22', 'WT.22', 'KO.23', 'WT.23', 'KO.24', 'WT.24'), each = 47240)
```
...
```{r}
shiny::selectInput('cell', 'Select the Cell type:', unique(to_plot_all$celltype))
# Define user input from a list of cell type
```
...
```{r}
shiny::selectInput('data_type', 'Select the type of data:', colnames(to_plot_all)[2:6])
```
...
```{r}
# Creating output with interactive histogram
output$lfq.all.Plot <- renderPlotly({
    selected_cell <- input$cell
    selected_data <- input$data_type
    
    # Filter data for selected gene
    filtered_data <- to_plot_all |>
        tidyr::pivot_longer(2:6, names_to='Type', values_to = 'LFQ') |>
        dplyr::filter(celltype == selected_cell & Type == selected_data)
    
    p <- plotly::plot_ly(data = filtered_data, x = ~LFQ, split = ~method,
                    type = 'histogram', opacity = 0.5, histfunc = 'count', xbins = 20) |>
        plotly::layout(
            title = paste('Distributions of the', selected_data, 'Normalized Data for', selected_cell),
            xaxis = list(title=paste('LFQ', selected_cell)),
            yaxis = list(title='Frequency'),
            barmode = 'overlay'
        )
})
...
cv.eig <- data.frame(KO=NA, WT=NA)
cv.eig[as.numeric(gsub('prot_', '', rownames(LFQ_KO.eigen))),] <- data.frame(KO=apply(2^LFQ_KO.eigen, 1, cv), WT=apply(2^LFQ_WT.eigen, 1, cv))

plotoneviolin(data.frame(
    Original = cv.before$KO,
    Z.Score = cv.standard$KO,
    Min.Max = cv.minmax$KO,
    Median.Scaling = cv.med$KO,
    MAD.Scaling = cv.mad$KO,
    Linear.Regression = cv.lm$KO,
    VSN = cv.vsn$KO,
    EigenMS = cv.eig$KO
...
plotoneviolin(data.frame(
    ...
    Z.Score = cv.standard$WT,
    Min.Max = cv.minmax$WT,
    Median.Scaling = cv.med$WT,
    MAD.Scaling = cv.mad$WT,
    Linear.Regression = cv.lm$WT,
    VSN = cv.vsn$WT,
    EigenMS = cv.eig$WT
...
plotoneviolin(data.frame(
    ...
    Z.Score = cv.standard,
    Min.Max = cv.minmax,
    Median.Scaling = cv.med,
    MAD.Scaling = cv.mad,
    Linear.Regression = cv.lm,
    VSN = cv.vsn,
    EigenMS = cv.eig
    ), 'LFQ CV [%] for Samples') +
...
plotoneviolin(data.frame(
    Original = cv_imp.before$KO,
    Z.Score = cv_imp.standard$KO,
    Min.Max = cv_imp.minmax$KO,
    Median.Scaling = cv_imp.med$KO,
    MAD.Scaling = cv_imp.mad$KO,
    Linear.Regression = cv_imp.lm$KO,
    VSN = cv_imp.vsn$KO,
    EigenMS = cv_imp.eig$KO
    ), 'LFQ CV [%] for KO Samples') + 
    ggplot2::geom_hline(yintercept = c(15,25), linetype=c(2,2), color=c('red','red'), alpha=c(0.7,0.7))

plotoneviolin(data.frame(
    Original = cv_imp.before$WT,
    Z.Score = cv_imp.standard$WT,
    Min.Max = cv_imp.minmax$WT,
    Median.Scaling = cv_imp.med$WT,
    MAD.Scaling = cv_imp.mad$WT,
    Linear.Regression = cv_imp.lm$WT,
    VSN = cv_imp.vsn$WT,
    EigenMS = cv_imp.eig$WT
    ), 'LFQ CV [%] for WT Samples') + 
    ggplot2::geom_hline(yintercept = c(15,25), linetype=c(2,2), color=c('red','red'), alpha=c(0.7,0.7))

plotoneviolin(data.frame(
    Original = cv_imp.before,
    Z.Score = cv_imp.standard,
    Min.Max = cv_imp.minmax,
    Median.Scaling = cv_imp.med,
    MAD.Scaling = cv_imp.mad,
    Linear.Regression = cv_imp.lm,
    VSN = cv_imp.vsn,
    EigenMS = cv_imp.eig
    ), 'LFQ CV [%] for Samples') +
    ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 45, hjust=1)) + 
...
plotoneviolin(data.frame(
    Original = cv_RF.before$KO,
    Z.Score = cv_RF.standard$KO,
    Min.Max = cv_RF.minmax$KO,
    Median.Scaling = cv_RF.med$KO,
    MAD.Scaling = cv_RF.mad$KO,
    Linear.Regression = cv_RF.lm$KO,
    VSN = cv_RF.vsn$KO,
    EigenMS = cv_RF.eig$KO
    ), 'LFQ CV [%] for KO Samples') + 
    ggplot2::geom_hline(yintercept = c(15,25), linetype=c(2,2), color=c('red','red'), alpha=c(0.7,0.7))

plotoneviolin(data.frame(
    Original = cv_RF.before$WT,
    Z.Score = cv_RF.standard$WT,
    Min.Max = cv_RF.minmax$WT,
    Median.Scaling = cv_RF.med$WT,
    MAD.Scaling = cv_RF.mad$WT,
    Linear.Regression = cv_RF.lm$WT,
    VSN = cv_RF.vsn$WT,
    EigenMS = cv_RF.eig$WT
    ), 'LFQ CV [%] for WT Samples') + 
    ggplot2::geom_hline(yintercept = c(15,25), linetype=c(2,2), color=c('red','red'), alpha=c(0.7,0.7))

plotoneviolin(data.frame(
    Original = cv_RF.before,
    Z.Score = cv_RF.standard,
    Min.Max = cv_RF.minmax,
    Median.Scaling = cv_RF.med,
    MAD.Scaling = cv_RF.mad,
    Linear.Regression = cv_RF.lm,
    VSN = cv_RF.vsn,
    EigenMS = cv_RF.eig
    ), 'LFQ CV [%] for Samples') +
    ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 45, hjust=1)) + 
    ggplot2::geom_hline(yintercept = c(15,25), linetype=2, color='red', alpha=0.7) + 
    ggplot2::geom_hline(yintercept = c(15,25), linetype=c(2,2), color=c('red','red'), alpha=c(0.7,0.7))
```
...
plotoneviolin(data.frame(
    Original = cv_kNN.before$KO,
    Z.Score = cv_kNN.standard$KO,
    Min.Max = cv_kNN.minmax$KO,
    Median.Scaling = cv_kNN.med$KO,
    MAD.Scaling = cv_kNN.mad$KO,
    Linear.Regression = cv_kNN.lm$KO,
    VSN = cv_kNN.vsn$KO,
    EigenMS = cv_kNN.eig$KO
    ), 'LFQ CV [%] for KO Samples') + 
    ggplot2::geom_hline(yintercept = c(15,25), linetype=c(2,2), color=c('red','red'), alpha=c(0.7,0.7))

plotoneviolin(data.frame(
    Original = cv_kNN.before$WT,
    Z.Score = cv_kNN.standard$WT,
    Min.Max = cv_kNN.minmax$WT,
    Median.Scaling = cv_kNN.med$WT,
    MAD.Scaling = cv_kNN.mad$WT,
    Linear.Regression = cv_kNN.lm$WT,
    VSN = cv_kNN.vsn$WT,
    EigenMS = cv_kNN.eig$WT
    ), 'LFQ CV [%] for WT Samples') + 
    ggplot2::geom_hline(yintercept = c(15,25), linetype=c(2,2), color=c('red','red'), alpha=c(0.7,0.7))

plotoneviolin(data.frame(
    Original = cv_kNN.before,
    Z.Score = cv_kNN.standard,
    Min.Max = cv_kNN.minmax,
    Median.Scaling = cv_kNN.med,
    MAD.Scaling = cv_kNN.mad,
    Linear.Regression = cv_kNN.lm,
    VSN = cv_kNN.vsn,
    EigenMS = cv_kNN.eig
    ), 'LFQ CV [%] for Samples') +
    ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 45, hjust=1)) + 
...
plotoneviolin(data.frame(
    ...
    Z.Score = cv_ludovic.standard$KO,
    Min.Max = cv_ludovic.minmax$KO,
    Median.Scaling = cv_ludovic.med$KO,
    MAD.Scaling = cv_ludovic.mad$KO,
    Linear.Regression = cv_ludovic.lm$KO,
    VSN = cv_ludovic.vsn$KO,
...
plotoneviolin(data.frame(
    ...
    Z.Score = cv_ludovic.standard$WT,
    Min.Max = cv_ludovic.minmax$WT,
    Median.Scaling = cv_ludovic.med$WT,
    MAD.Scaling = cv_ludovic.mad$WT,
    Linear.Regression = cv_ludovic.lm$WT,
    VSN = cv_ludovic.vsn$WT,
    EigenMS = cv_ludovic.eig$WT
    ), 'LFQ CV [%] for WT Samples') + 
    ggplot2::geom_hline(yintercept = c(15,25), linetype=c(2,2), color=c('red','red'), alpha=c(0.7,0.7))

plotoneviolin(data.frame(
    Original = cv_ludovic.before,
    Z.Score = cv_ludovic.standard,
    Min.Max = cv_ludovic.minmax,
    Median.Scaling = cv_ludovic.med,
    MAD.Scaling = cv_ludovic.mad,
    Linear.Regression = cv_ludovic.lm,
    VSN = cv_ludovic.vsn,
    EigenMS = cv_ludovic.eig
    ), 'LFQ CV [%] for Samples') +
    ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 45, hjust=1)) + 
    ggplot2::geom_hline(yintercept = c(15,25), linetype=c(2,2), color=c('red','red'), alpha=c(0.7,0.7))
```
...
```{r CV6, fig.width=10}
# CV calculation for all normalization methods with imputed data using ludovic
cv_shift.rf.before <- data.frame(KO=apply(2^LFQ_KO_shift.rf, 1, cv), WT=apply(2^LFQ_WT_shift.rf,1,cv))
cv_shift.rf.standard <- data.frame(KO=apply(2^LFQ_KO_shift.rf.standard, 1, cv), WT=apply(2^LFQ_WT_shift.rf.standard, 1, cv))
cv_shift.rf.minmax <- data.frame(KO=apply(2^LFQ_KO_shift.rf.minmax, 1, cv), WT= apply(2^LFQ_WT_shift.rf.minmax, 1, cv))
cv_shift.rf.med <- data.frame(KO=apply(2^LFQ_KO_shift.rf.med, 1, cv), WT=apply(2^LFQ_WT_shift.rf.med, 1, cv))
cv_shift.rf.mad <- data.frame(KO=apply(2^LFQ_KO_shift.rf.mad, 1, cv), WT=apply(2^LFQ_WT_shift.rf.mad, 1, cv))
cv_shift.rf.lm <- data.frame(KO=apply(2^LFQ_KO_shift.rf.lm, 1, cv), WT=apply(2^LFQ_WT_shift.rf.lm, 1, cv))
cv_shift.rf.vsn <- data.frame(KO=apply(2^LFQ_KO_shift.rf.vsn, 1, cv), WT=apply(2^LFQ_WT_shift.rf.vsn, 1, cv))
cv_shift.rf.eig <- data.frame(KO=apply(2^LFQ_KO_shift.rf.eigen, 1, cv), WT=apply(2^LFQ_WT_shift.rf.eigen, 1, cv))

plotoneviolin(data.frame(
    Original = cv_shift.rf.before$KO,
    Z.Score = cv_shift.rf.standard$KO,
    Min.Max = cv_shift.rf.minmax$KO,
    Median.Scaling = cv_shift.rf.med$KO,
    MAD.Scaling = cv_shift.rf.mad$KO,
    Linear.Regression = cv_shift.rf.lm$KO,
    VSN = cv_shift.rf.vsn$KO,
    EigenMS = cv_shift.rf.eig$KO
    ), 'LFQ CV [%] for KO Samples') + 
    ggplot2::geom_hline(yintercept = c(15,25), linetype=c(2,2), color=c('red','red'), alpha=c(0.7,0.7))

plotoneviolin(data.frame(
    Original = cv_shift.rf.before$WT,
    Z.Score = cv_shift.rf.standard$WT,
    Min.Max = cv_shift.rf.minmax$WT,
    Median.Scaling = cv_shift.rf.med$WT,
    MAD.Scaling = cv_shift.rf.mad$WT,
    Linear.Regression = cv_shift.rf.lm$WT,
    VSN = cv_shift.rf.vsn$WT,
    EigenMS = cv_shift.rf.eig$WT
    ), 'LFQ CV [%] for WT Samples') + 
    ggplot2::geom_hline(yintercept = c(15,25), linetype=c(2,2), color=c('red','red'), alpha=c(0.7,0.7))

plotoneviolin(data.frame(
    Original = cv_shift.rf.before,
    Z.Score = cv_shift.rf.standard,
    Min.Max = cv_shift.rf.minmax,
    Median.Scaling = cv_shift.rf.med,
    MAD.Scaling = cv_shift.rf.mad,
    Linear.Regression = cv_shift.rf.lm,
    VSN = cv_shift.rf.vsn,
    EigenMS = cv_shift.rf.eig
    ), 'LFQ CV [%] for Samples') +
    ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 45, hjust=1)) + 
    ggplot2::geom_hline(yintercept = c(15,25), linetype=c(2,2), color=c('red','red'), alpha=c(0.7,0.7))
```
...
```{r show CV5}
ggarrange(
    plotoneviolin(cv_shift.rf.before, 'Original'),
    plotoneviolin(cv_shift.rf.standard, 'Z-Score'),
    plotoneviolin(cv_shift.rf.minmax, 'Min-Max'),
    plotoneviolin(cv_shift.rf.med, 'Median Scaling'),
    nrow=2,ncol=2
)

ggarrange(
    plotoneviolin(cv_shift.rf.mad, 'MAD Scaling'),
    plotoneviolin(cv_shift.rf.lm, 'Linear Regression'),
    plotoneviolin(cv_shift.rf.vsn, 'VSN'),
    plotoneviolin(cv_shift.rf.eig, 'EigenMS'),
    nrow=2,ncol=2
)
```
...
# bind all calculated CV values into one data frame - shift dist + RF
cv_shift.rf_all <- data.frame(cv_shift.rf.before, cv_shift.rf.standard, cv_shift.rf.minmax, cv_shift.rf.med,
                         cv_shift.rf.mad, cv_shift.rf.lm, cv_shift.rf.vsn, cv_shift.rf.eig)
colnames(cv_shift.rf_all) <- c('Before_KO', 'Before_WT', 'Z-score_KO', 'Z-score_WT',
                          'MinMax_KO', 'MinMax_WT', 'Median_KO', 'Median_WT', 
                          'MAD_KO', 'MAD_WT', 'Linear_KO', 'Linear_WT', 
                          'VSN_KO', 'VSN_WT', 'EigenMS_KO', 'EigenMS_WT')
```
...
standard_df <- na.omit(standard_df)
...
duplicates <- which(lapply(1:nrow(standard_df_kNN), function(i) length(unique(as.numeric(standard_df_kNN[i,])))) != 6)
standard_df_kNN <- standard_df_kNN
...
## t-test 24th rep out {.tabset}

In this section we calculate the t-test again, but without the 24th replicate.
...
```{r p-value rawdata 24 rep out}
# prepare data for t-test
df_24out <- lfq[,-c(3,6)]
df_24out <- na.omit(df_24out)

# calculate the p-values for our data
pvalue_24out <- df_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(df_24out, 1, ttest, 
                  grp1=grep("KO", colnames(df_24out)), 
                  grp2=grep("WT", colnames(df_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column 
m <- mean(pvalue_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_24out <- ggplot(data = pvalue_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Non-Imputed - p.value') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 4, color = "black")
```

```{r p-value impdata 24 rep out}
lfq_imp_24out <- lfq_imp[, -c(3,6)]

# calculate the p-values for our data
pvalue_imp_24out <- lfq_imp_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(lfq_imp_24out, 1, ttest, 
                  grp1=grep("KO", colnames(lfq_imp_24out)), 
                  grp2=grep("WT", colnames(lfq_imp_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column 
m <- mean(pvalue_imp_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_imp_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_imp_24out <- ggplot(data = pvalue_imp_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Imputed Mati Code - p.value') + theme(legend.position = 'none') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 3.5, color = "black")
```

```{r p-value RFdata 24 rep out}
imputed_RF_24out <- imputed_RF[,-c(3,6)]

# calculate the p-values for our data
pvalue_RF_24out <- imputed_RF_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(imputed_RF_24out, 1, ttest, 
                  grp1=grep("KO", colnames(imputed_RF_24out)), 
                  grp2=grep("WT", colnames(imputed_RF_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column 
m <- mean(pvalue_RF_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_RF_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_RF_24out <- ggplot(data = pvalue_RF_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Imputed RF - p.value') + theme(legend.position = 'none') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 3.5, color = "black")
```

```{r p-value kNN data 24 rep out}
# handling with duplicates
df_kNN_24out <- df_kNN[,-c(3,6)]

# calculate the p-values for our data
pvalue_kNN_24out <- df_kNN_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(df_kNN_24out, 1, ttest, 
                  grp1=grep("KO", colnames(df_kNN_24out)), 
                  grp2=grep("WT", colnames(df_kNN_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column 
m <- mean(pvalue_kNN_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_kNN_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_kNN_24out <- ggplot(data = pvalue_kNN_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Imputed kNN - p.value') + theme(legend.position = 'none') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 3.5, color = "black")
```

```{r p-value ludovic data 24 rep out}
# calculate the p-values for our data
pvalue_ludovic_24out <- imputed_ludovic[,-c(3,6)] |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(imputed_ludovic[,-c(3,6)], 1, ttest, 
                  grp1=grep("KO", colnames(imputed_ludovic[,-c(3,6)])), 
                  grp2=grep("WT", colnames(imputed_ludovic[,-c(3,6)])))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column 
m <- mean(pvalue_ludovic_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_ludovic_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_ludovic_24out <- ggplot(data = pvalue_ludovic_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Imputed ludovic - p.value') + theme(legend.position = 'none') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 3.5, color = "black")
```
...
```{r p-value raw data standardization 24 rep out}
# prepare data for t-test
standard_df_24out <- standard_df[,-c(3,6)]

# calculate the p-values for our data
pvalue_standard_24out <- standard_df_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(standard_df_24out, 1, ttest, 
                  grp1=grep("KO", colnames(standard_df_24out)), 
                  grp2=grep("WT", colnames(standard_df_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column 
m <- mean(pvalue_standard_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_standard_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_standard_24out <- ggplot(data = pvalue_standard_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Non-Imputed - p.value') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 4, color = "black")
```

```{r p-value imputed data standardization 24 rep out}
# prepare data for t-test
standard_df_imp_24out <- standard_df_imp[,-c(3,6)]

# calculate the p-values for our data
pvalue_standard_imp_24out <- standard_df_imp_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(standard_df_imp_24out, 1, ttest, 
                  grp1=grep("KO", colnames(standard_df_imp_24out)), 
                  grp2=grep("WT", colnames(standard_df_imp_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column 
m <- mean(pvalue_standard_imp_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_standard_imp_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_standard_imp_24out <- ggplot(data = pvalue_standard_imp_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Imputed Mati Code - p.value') + theme(legend.position = 'none') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 3.5, color = "black")
```

```{r p-value rf data standardization 24 rep out}
# prepare data for t-test
standard_df_RF_24out <- standard_df_RF[-c(3,6)]

# calculate the p-values for our data
pvalue_standard_RF_24out <- standard_df_RF_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(standard_df_RF_24out, 1, ttest, 
                  grp1=grep("KO", colnames(standard_df_RF_24out)), 
                  grp2=grep("WT", colnames(standard_df_RF_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column 
m <- mean(pvalue_standard_RF_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_standard_RF_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_standard_RF_24out <- ggplot(data = pvalue_standard_RF_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Imputed RF - p.value') + theme(legend.position = 'none') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 3.5, color = "black")
```

```{r p-value kNN data standardization 24 rep out}
# prepare data for t-test
standard_df_kNN_24out <- standard_df_kNN[-c(3,6)]

# calculate the p-values for our data
pvalue_standard_kNN_24out <- standard_df_kNN_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(standard_df_kNN_24out, 1, ttest, 
                  grp1=grep("KO", colnames(standard_df_kNN_24out)), 
                  grp2=grep("WT", colnames(standard_df_kNN_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column 
m <- mean(pvalue_standard_kNN_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_standard_kNN_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_standard_kNN_24out <- ggplot(data = pvalue_standard_kNN_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Imputed kNN - p.value') + theme(legend.position = 'none') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 3.5, color = "black")
```

```{r p-value ludovic data standardization 24 rep out}
# prepare data for t-test
standard_df_ludovic_24out <- standard_df_ludovic[-c(3,6)]

# calculate the p-values for our data
pvalue_standard_ludovic_24out <- standard_df_ludovic_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(standard_df_ludovic_24out, 1, ttest, 
                  grp1=grep("KO", colnames(standard_df_ludovic_24out)), 
                  grp2=grep("WT", colnames(standard_df_ludovic_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column 
m <- mean(pvalue_standard_ludovic_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_standard_ludovic_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_standard_ludovic_24out <- ggplot(data = pvalue_standard_ludovic_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Imputed ludovic - p.value') + theme(legend.position = 'none') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 3.5, color = "black")
```
...
```{r p-value raw data minmax 24 rep out}
# prepare data for t-test
minmax_df_24out <- minmax_df[-c(3,6)]

# calculate the p-values for our data
pvalue_minmax_24out <- minmax_df_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(minmax_df_24out, 1, ttest, 
                  grp1=grep("KO", colnames(minmax_df_24out)), 
                  grp2=grep("WT", colnames(minmax_df_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column 
m <- mean(pvalue_minmax_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_minmax_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_minmax_24out <- ggplot(data = pvalue_minmax_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Non-Imputed - p.value') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 4, color = "black")
```

```{r p-value imputed data minmax 24 rep out}
# prepare data for t-test
minmax_df_imp_24out <- minmax_df_imp[-c(3,6)]

# calculate the p-values for our data
pvalue_minmax_imp_24out <- minmax_df_imp_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(minmax_df_imp_24out, 1, ttest, 
                  grp1=grep("KO", colnames(minmax_df_imp_24out)), 
                  grp2=grep("WT", colnames(minmax_df_imp_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column 
m <- mean(pvalue_minmax_imp_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_minmax_imp_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_minmax_imp_24out <- ggplot(data = pvalue_minmax_imp_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Imputed Mati Code - p.value') + theme(legend.position = 'none') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 3.5, color = "black")
```

```{r p-value rf data minmax 24 rep out}
# prepare data for t-test
minmax_df_RF_24out <- minmax_df_RF[,-c(3,6)]

# calculate the p-values for our data
pvalue_minmax_RF_24out <- minmax_df_RF_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(minmax_df_RF_24out, 1, ttest, 
                  grp1=grep("KO", colnames(minmax_df_RF_24out)), 
                  grp2=grep("WT", colnames(minmax_df_RF_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column 
m <- mean(pvalue_minmax_RF_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_minmax_RF_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_minmax_RF_24out <- ggplot(data = pvalue_minmax_RF_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Imputed RF - p.value') + theme(legend.position = 'none') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 3.5, color = "black")
```

```{r p-value kNN data minmax 24 rep out}
# prepare data for t-test
minmax_df_kNN_24out <- minmax_df_kNN[,-c(3,6)]

# calculate the p-values for our data
pvalue_minmax_kNN_24out <- minmax_df_kNN_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(minmax_df_kNN_24out, 1, ttest, 
                  grp1=grep("KO", colnames(minmax_df_kNN_24out)), 
                  grp2=grep("WT", colnames(minmax_df_kNN_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column 
m <- mean(pvalue_minmax_kNN_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_minmax_kNN_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_minmax_kNN_24out <- ggplot(data = pvalue_minmax_kNN_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Imputed kNN - p.value') + theme(legend.position = 'none') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 3.5, color = "black")
```

```{r p-value ludovic data minmax 24 rep out}
# prepare data for t-test
minmax_df_ludovic_24out <- minmax_df_ludovic[,-c(3,6)]

# calculate the p-values for our data
pvalue_minmax_ludovic_24out <- minmax_df_ludovic_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(minmax_df_ludovic_24out, 1, ttest, 
                  grp1=grep("KO", colnames(minmax_df_ludovic_24out)), 
                  grp2=grep("WT", colnames(minmax_df_ludovic_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column 
m <- mean(pvalue_minmax_ludovic$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_minmax_ludovic$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_minmax_ludovic_24out <- ggplot(data = pvalue_minmax_ludovic, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Imputed ludovic - p.value') + theme(legend.position = 'none') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 3.5, color = "black")
```
...
```{r}
ggarrange(
    hist_p_minmax_imp_24out,
    hist_p_minmax_RF_24out,
    hist_p_minmax_kNN_24out,
    hist_p_minmax_ludovic_24out,
    nrow=2,ncol=2
)
```
...
```{r p-value raw data median 24 rep out}
# prepare data for t-test
med_df_24out <- med_df[,-c(3,6)]

# calculate the p-values for our data
pvalue_med_24out <- med_df_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(med_df_24out, 1, ttest, 
                  grp1=grep("KO", colnames(med_df_24out)), 
                  grp2=grep("WT", colnames(med_df_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column 
m <- mean(pvalue_med_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_med_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_med_24out <- ggplot(data = pvalue_med_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Non-Imputed - p.value') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 4, color = "black")
```

```{r p-value imputed data median 24 rep out}
# prepare data for t-test
med_df_imp_24out <- med_df_imp[,-c(3,6)]

# calculate the p-values for our data
pvalue_med_imp_24out <- med_df_imp_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(med_df_imp_24out, 1, ttest, 
                  grp1=grep("KO", colnames(med_df_imp_24out)), 
                  grp2=grep("WT", colnames(med_df_imp_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column 
m <- mean(pvalue_med_imp_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_med_imp_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_med_imp_24out <- ggplot(data = pvalue_med_imp_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Imputed Mati Code - p.value') + theme(legend.position = 'none') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 3.5, color = "black")
```

```{r p-value rf data median 24 rep out}
# prepare data for t-test
med_df_RF_24out <- med_df_RF[,-c(3,6)]

# calculate the p-values for our data
pvalue_med_RF_24out <- med_df_RF_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(med_df_RF_24out, 1, ttest, 
                  grp1=grep("KO", colnames(med_df_RF_24out)), 
                  grp2=grep("WT", colnames(med_df_RF_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column 
m <- mean(pvalue_med_RF_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_med_RF_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_med_RF_24out <- ggplot(data = pvalue_med_RF_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Imputed RF - p.value') + theme(legend.position = 'none') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 3.5, color = "black")
```

```{r p-value kNN data median 24 rep out}
# prepare data for t-test
med_df_kNN_24out <- med_df_kNN[,-c(3,6)]

# calculate the p-values for our data
pvalue_med_kNN_24out <- med_df_kNN_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(med_df_kNN_24out, 1, ttest, 
                  grp1=grep("KO", colnames(med_df_kNN_24out)), 
                  grp2=grep("WT", colnames(med_df_kNN_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column 
m <- mean(pvalue_med_kNN_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_med_kNN_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_med_kNN_24out <- ggplot(data = pvalue_med_kNN_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Imputed kNN - p.value') + theme(legend.position = 'none') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 3.5, color = "black")
```

```{r p-value ludovic data median 24 rep out}
# prepare data for t-test
med_df_ludovic_24out <- med_df_ludovic[,-c(3,6)]

# calculate the p-values for our data
pvalue_med_ludovic_24out <- med_df_ludovic_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(med_df_ludovic_24out, 1, ttest, 
                  grp1=grep("KO", colnames(med_df_ludovic_24out)), 
                  grp2=grep("WT", colnames(med_df_ludovic_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column 
m <- mean(pvalue_med_ludovic_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_med_ludovic_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_med_ludovic_24out <- ggplot(data = pvalue_med_ludovic_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Imputed ludovic - p.value') + theme(legend.position = 'none') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 3.5, color = "black")
```
...
```{r}
ggarrange(
    hist_p_med_imp_24out,
    hist_p_med_RF_24out,
    hist_p_med_kNN_24out,
    hist_p_med_ludovic_24out,
    nrow=2,ncol=2
)
```
...
```{r p-value raw data mad 24 rep out}
# prepare data for t-test
mad_df_24out <- mad_df[,-c(3,6)]

# calculate the p-values for our data
pvalue_mad_24out <- mad_df_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(mad_df_24out, 1, ttest, 
                  grp1=grep("KO", colnames(mad_df_24out)), 
                  grp2=grep("WT", colnames(mad_df_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column 
m <- mean(pvalue_mad_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_mad_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_mad_24out <- ggplot(data = pvalue_mad_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Non-Imputed - p.value') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 4, color = "black")
```

```{r p-value imputed data mad 24 rep out}
# prepare data for t-test
mad_df_imp_24out <- mad_df_imp[,-c(3,6)]

# calculate the p-values for our data
pvalue_mad_imp_24out <- mad_df_imp_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(mad_df_imp_24out, 1, ttest, 
                  grp1=grep("KO", colnames(mad_df_imp_24out)), 
                  grp2=grep("WT", colnames(mad_df_imp_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column 
m <- mean(pvalue_mad_imp_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_mad_imp_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_mad_imp_24out <- ggplot(data = pvalue_mad_imp_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Imputed Mati Code - p.value') + theme(legend.position = 'none') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 3.5, color = "black")
```

```{r p-value rf data mad 24 rep out}
# prepare data for t-test
mad_df_RF_24out <- mad_df_RF[,-c(3,6)]

# calculate the p-values for our data
pvalue_mad_RF_24out <- mad_df_RF_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(mad_df_RF_24out, 1, ttest, 
                  grp1=grep("KO", colnames(mad_df_RF_24out)), 
                  grp2=grep("WT", colnames(mad_df_RF_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column
m <- mean(pvalue_mad_RF_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_mad_RF_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_mad_RF_24out <- ggplot(data = pvalue_mad_RF_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Imputed RF - p.value') + theme(legend.position = 'none') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 3.5, color = "black")
```

```{r p-value kNN data mad 24 rep out}
# prepare data for t-test
mad_df_kNN_24out <- mad_df_kNN[,-c(3,6)]

# calculate the p-values for our data
pvalue_mad_kNN_24out <- mad_df_kNN_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(mad_df_kNN_24out, 1, ttest, 
                  grp1=grep("KO", colnames(mad_df_kNN_24out)), 
                  grp2=grep("WT", colnames(mad_df_kNN_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column
m <- mean(pvalue_mad_kNN_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_mad_kNN_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_mad_kNN_24out <- ggplot(data = pvalue_mad_kNN_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Imputed kNN - p.value') + theme(legend.position = 'none') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 3.5, color = "black")
```

```{r p-value ludovic data mad 24 rep out}
# prepare data for t-test
mad_df_ludovic_24out <- mad_df_ludovic[,-c(3,6)]

# calculate the p-values for our data
pvalue_mad_ludovic_24out <- mad_df_ludovic_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(mad_df_ludovic_24out, 1, ttest, 
                  grp1=grep("KO", colnames(mad_df_ludovic_24out)), 
                  grp2=grep("WT", colnames(mad_df_ludovic_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column
m <- mean(pvalue_mad_ludovic_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_mad_ludovic_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_mad_ludovic_24out <- ggplot(data = pvalue_mad_ludovic_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Imputed ludovic - p.value') + theme(legend.position = 'none') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 3.5, color = "black")
```
...
```{r}
ggarrange(
    hist_p_mad_imp_24out,
    hist_p_mad_RF_24out,
    hist_p_mad_kNN_24out,
    hist_p_mad_ludovic_24out,
    nrow=2,ncol=2
)
```
...
```{r p-value raw data lm24 rep out}
# prepare data for t-test
lm_df_24out <- lm_df[,-c(3,6)]

# calculate the p-values for our data
pvalue_lm_24out <- lm_df_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(lm_df_24out, 1, ttest, 
                  grp1=grep("KO", colnames(lm_df_24out)), 
                  grp2=grep("WT", colnames(lm_df_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column
m <- mean(pvalue_lm_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_lm_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_lm_24out <- ggplot(data = pvalue_lm_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Non-Imputed - p.value') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 4, color = "black")
```

```{r p-value imputed data lm24 rep out}
# prepare data for t-test
lm_df_imp_24out <- lm_df_imp[,-c(3,6)]

# calculate the p-values for our data
pvalue_lm_imp_24out <- lm_df_imp_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(lm_df_imp_24out, 1, ttest, 
                  grp1=grep("KO", colnames(lm_df_imp_24out)), 
                  grp2=grep("WT", colnames(lm_df_imp_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column
m <- mean(pvalue_lm_imp_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_lm_imp_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_lm_imp_24out <- ggplot(data = pvalue_lm_imp_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Imputed Mati Code - p.value') + theme(legend.position = 'none') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 3.5, color = "black")
```

```{r p-value rf data lm24 rep out}
# prepare data for t-test
lm_df_RF_24out <- lm_df_RF[,-c(3,6)]

# calculate the p-values for our data
pvalue_lm_RF_24out <- lm_df_RF_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(lm_df_RF_24out, 1, ttest, 
                  grp1=grep("KO", colnames(lm_df_RF_24out)), 
                  grp2=grep("WT", colnames(lm_df_RF_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column
m <- mean(pvalue_lm_RF_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_lm_RF_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_lm_RF_24out <- ggplot(data = pvalue_lm_RF_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Imputed RF - p.value') + theme(legend.position = 'none') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 3.5, color = "black")
```

```{r p-value kNN data lm24 rep out}
# prepare data for t-test
lm_df_kNN_24out <- lm_df_kNN[,-c(3,6)]

# calculate the p-values for our data
pvalue_lm_kNN_24out <- lm_df_kNN_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(lm_df_kNN_24out, 1, ttest, 
                  grp1=grep("KO", colnames(lm_df_kNN_24out)), 
                  grp2=grep("WT", colnames(lm_df_kNN_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column
m <- mean(pvalue_lm_kNN_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_lm_kNN_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_lm_kNN_24out <- ggplot(data = pvalue_lm_kNN_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Imputed kNN - p.value') + theme(legend.position = 'none') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 3.5, color = "black")
```

```{r p-value ludovic data lm24 rep out}
# prepare data for t-test
lm_df_ludovic_24out <- lm_df_ludovic[,-c(3,6)]

# calculate the p-values for our data
pvalue_lm_ludovic_24out <- lm_df_ludovic_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(lm_df_ludovic_24out, 1, ttest, 
                  grp1=grep("KO", colnames(lm_df_ludovic_24out)), 
                  grp2=grep("WT", colnames(lm_df_ludovic_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column
m <- mean(pvalue_lm_ludovic_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_lm_ludovic_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_lm_ludovic_24out <- ggplot(data = pvalue_lm_ludovic_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Imputed ludovic - p.value') + theme(legend.position = 'none') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 3.5, color = "black")
```
...
```{r p-value raw data vsn 24 rep out}
# prepare data for t-test
vsn_df_24out <- vsn_df[,-c(3,6)]

# calculate the p-values for our data
pvalue_vsn_24out <- vsn_df_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(vsn_df_24out, 1, ttest, 
                  grp1=grep("KO", colnames(vsn_df_24out)), 
                  grp2=grep("WT", colnames(vsn_df_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column
m <- mean(pvalue_vsn_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_vsn_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_vsn_24out <- ggplot(data = pvalue_vsn_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Non-Imputed - p.value') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 4, color = "black")
```

```{r p-value imputed data vsn 24 rep out}
# prepare data for t-test
vsn_df_imp_24out <- vsn_df_imp[,-c(3,6)]

# calculate the p-values for our data
pvalue_vsn_imp_24out <- vsn_df_imp_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(vsn_df_imp_24out, 1, ttest, 
                  grp1=grep("KO", colnames(vsn_df_imp_24out)), 
                  grp2=grep("WT", colnames(vsn_df_imp_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column
m <- mean(pvalue_vsn_imp_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_vsn_imp_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_vsn_imp_24out <- ggplot(data = pvalue_vsn_imp_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Imputed Mati Code - p.value') + theme(legend.position = 'none') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 3.5, color = "black")
```

```{r p-value rf data vsn 24 rep out}
# prepare data for t-test
vsn_df_RF_24out <- vsn_df_RF[,-c(3,6)]

# calculate the p-values for our data
pvalue_vsn_RF_24out <- vsn_df_RF_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(vsn_df_RF_24out, 1, ttest, 
                  grp1=grep("KO", colnames(vsn_df_RF_24out)), 
                  grp2=grep("WT", colnames(vsn_df_RF_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column
m <- mean(pvalue_vsn_RF_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_vsn_RF_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_vsn_RF_24out <- ggplot(data = pvalue_vsn_RF_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Imputed RF - p.value') + theme(legend.position = 'none') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 3.5, color = "black")
```

```{r p-value kNN data vsn 24 rep out}
# prepare data for t-test
vsn_df_kNN_24out <- vsn_df_kNN[,-c(3,6)]

# calculate the p-values for our data
pvalue_vsn_kNN_24out <- vsn_df_kNN_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(vsn_df_kNN_24out, 1, ttest, 
                  grp1=grep("KO", colnames(vsn_df_kNN_24out)), 
                  grp2=grep("WT", colnames(vsn_df_kNN_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column
m <- mean(pvalue_vsn_kNN_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_vsn_kNN_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_vsn_kNN_24out <- ggplot(data = pvalue_vsn_kNN_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Imputed kNN - p.value') + theme(legend.position = 'none') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 3.5, color = "black")
```

```{r p-value ludovic data vsn 24 rep out}
# prepare data for t-test
vsn_df_ludovic_24out <- vsn_df_ludovic[,-c(3,6)]

# calculate the p-values for our data
pvalue_vsn_ludovic_24out <- vsn_df_ludovic_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(vsn_df_ludovic_24out, 1, ttest, 
                  grp1=grep("KO", colnames(vsn_df_ludovic_24out)), 
                  grp2=grep("WT", colnames(vsn_df_ludovic_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column
m <- mean(pvalue_vsn_ludovic_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_vsn_ludovic_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_vsn_ludovic_24out <- ggplot(data = pvalue_vsn_ludovic_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Imputed ludovic - p.value') + theme(legend.position = 'none') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 3.5, color = "black")
```
...
```{r}
ggarrange(
    hist_p_vsn_imp_24out,
    hist_p_vsn_RF_24out,
    hist_p_vsn_kNN_24out,
    hist_p_vsn_ludovic_24out,
    nrow=2,ncol=2
)
```
...
```{r p-value raw data eigen 24 rep out}
# prepare data for t-test
eigen_df_24out <- eigen_df[,-c(3,6)]

# calculate the p-values for our data
pvalue_eigen_24out <- eigen_df_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(eigen_df_24out, 1, ttest, 
                  grp1=grep("KO", colnames(eigen_df_24out)), 
                  grp2=grep("WT", colnames(eigen_df_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column
m <- mean(pvalue_eigen_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_eigen_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_eigen_24out <- ggplot(data = pvalue_eigen_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Non-Imputed - p.value') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 4, color = "black")
```

```{r p-value imputed data eigen 24 rep out}
# prepare data for t-test
eigen_df_imp_24out <- eigen_df_imp[,-c(3,6)]

# calculate the p-values for our data
pvalue_eigen_imp_24out <- eigen_df_imp_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(eigen_df_imp_24out, 1, ttest, 
                  grp1=grep("KO", colnames(eigen_df_imp_24out)), 
                  grp2=grep("WT", colnames(eigen_df_imp_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column
m <- mean(pvalue_eigen_imp_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_eigen_imp_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_eigen_imp_24out <- ggplot(data = pvalue_eigen_imp_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Imputed Mati Code - p.value') + theme(legend.position = 'none') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 3.5, color = "black")
```

```{r p-value rf data eigen 24 rep out}
# prepare data for t-test
eigen_df_RF_24out <- eigen_df_RF[,-c(3,6)]

# calculate the p-values for our data
pvalue_eigen_RF_24out <- eigen_df_RF_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(eigen_df_RF_24out, 1, ttest, 
                  grp1=grep("KO", colnames(eigen_df_RF_24out)), 
                  grp2=grep("WT", colnames(eigen_df_RF_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column
m <- mean(pvalue_eigen_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_eigen_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_eigen_RF_24out <- ggplot(data = pvalue_eigen_RF_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Imputed RF - p.value') + theme(legend.position = 'none') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 3.5, color = "black")
```

```{r p-value kNN data eigen 24 rep out}
# The kNN method doesn't work well with imputation. In a few rows 
# we didn't have a unique value for each sample. Sometimes there was only one 
# value for all columns and sometimes there were two values, one for KO and the 
# other for WT. That's why we had to remove duplicates from data.
...
# prepare data for t-test
eigen_df_kNN_24out <- eigen_df_kNN[,-c(3,6,7)]

# calculate the p-values for our data
pvalue_eigen_kNN_24out <- eigen_df_kNN_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(eigen_df_kNN_24out, 1, ttest, 
                  grp1=grep("KO", colnames(eigen_df_kNN_24out)), 
                  grp2=grep("WT", colnames(eigen_df_kNN_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column
m <- mean(pvalue_eigen_kNN_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_eigen_kNN_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_eigen_kNN_24out <- ggplot(data = pvalue_eigen_kNN_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Imputed kNN - p.value') + theme(legend.position = 'none') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 3.5, color = "black")
```

```{r p-value ludovic data eigen 24 rep out}
# prepare data for t-test
eigen_df_ludovic_24out <- eigen_df_ludovic[,c(1:2, 4:5)]

# calculate the p-values for our data
pvalue_eigen_ludovic_24out <- eigen_df_ludovic_24out |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(eigen_df_ludovic_24out, 1, ttest, 
                  grp1=grep("KO", colnames(eigen_df_ludovic_24out)), 
                  grp2=grep("WT", colnames(eigen_df_ludovic_24out)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE)) # add a significance column
m <- mean(pvalue_eigen_ludovic_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # mean of p
me <- median(pvalue_eigen_ludovic_24out$p_OCIAD1_TOTALS, na.rm=TRUE) # median of p

# Plotting the histogram of the p-values filled with a significance info
hist_p_eigen_ludovic_24out <- ggplot(data = pvalue_eigen_ludovic_24out, aes(x = p_OCIAD1_TOTALS, fill=significant)) + 
    geom_histogram(bins = 100) + ggtitle('Imputed ludovic - p.value') + theme(legend.position = 'none') +
    annotate("text", x = Inf, y = Inf, label = paste("μ(p) =", round(m, 3), '\nmed(p) =', round(me,3)), 
             hjust = 1.1, vjust = 1.3, size = 3.5, color = "black")
```
...
```{r}
ggarrange(
    hist_p_eigen_imp_24out,
    hist_p_eigen_RF_24out,
    hist_p_eigen_kNN_24out,
    hist_p_eigen_ludovic_24out,
    nrow=2,ncol=2
)
```
...
```{r}
# extracting rows with non missing rows
miss <- apply(lfq[,1:6], 1, function(row) all(!is.na(row)))
# extracting eigen prots IDs for raw data
eig_prots_raw <- as.numeric(gsub('prot_', '',rownames(eigen_df)))
# extracting ludovic prots IDs which are in the raw and ludovic eigen data
lud_prots_to_keep <- as.numeric(gsub('prot_', '',rownames(eigen_df_ludovic)))

# Preparing columns
eigen_df_prep <- data.frame(KO.22=NA, KO.23=NA, KO.24=NA, WT.22=NA, WT.23=NA, WT.24=NA)
eigen_df_prep[eig_prots_raw,] <- eigen_df 

df_prep_lud <- data.frame(KO.22=NA, KO.23=NA, KO.24=NA, WT.22=NA, WT.23=NA, WT.24=NA)
df_prep_lud[lud_prots_to_keep,] <- imputed_ludovic[,3:8]

eigen_df_prep_lud <- data.frame(KO.22=NA, KO.23=NA, KO.24=NA, WT.22=NA, WT.23=NA, WT.24=NA)
eigen_df_prep_lud[lud_prots_to_keep,] <- LFQ_ludovic.eig_norm$norm_m

# Create a data frame to make interactive plot
data <- data.frame(
    Gene_Name = rep(protein.groups$`Gene names`, each=6),
    lfq[,1:6] |> 
    ...
    imputed_RF[,1:6] |> 
        tidyr::pivot_longer(cols=everything(), names_to = 'Batch3', values_to = 'RF'),
    as_tibble(LFQ_RF.eig_norm$norm_m) |> 
        tidyr::pivot_longer(cols=everything(), names_to = 'Batch4', values_to = 'Eigen.RF'),
    df_prep_lud |> 
    ...
    dplyr::select(Gene_Name, Batch, Raw, Eigen.Raw, RF, Eigen.RF, Ludovic, Eigen.Ludovic)

data[, 3:8][is.na(data[, 3:8])] <- 0
```
...
output$genePlot <- renderPlotly({
  ...
  plot_ly(data = filtered_data, type = "bar") |>
      ...
      add_trace(x = ~Batch, y = ~RF, name = 'RF') |>
      add_trace(x = ~Batch, y = ~Eigen.RF, name = 'Eigen RF') |>
      add_trace(x = ~Batch, y = ~Ludovic, name = 'Ludovic') |>
      add_trace(x = ~Batch, y = ~Eigen.Ludovic, name = 'Eigen Ludovic') |>
      layout(
      title = paste("Scatter Plot for Gene:", selected_gene),
...
```{r interactive bar-plot unlogged}
# Creating output with interactive bar plot
output$genePlotunlogged <- renderPlotly({
  selected_gene <- input$gene

  # Filter data for selected gene
  filtered_data <- data |>
      dplyr::filter(Gene_Name == selected_gene)
  filtered_data[,3:8] <- 2^filtered_data[,3:8]
  ...
  plot_ly(data = filtered_data, type = "bar") |>
      ...
      add_trace(x = ~Batch, y = ~Eigen.Ludovic, name = 'Eigen Ludovic') |>
      layout(
      title = paste("Scatter Plot for Gene:", selected_gene, '- unlogged data'),
      xaxis = list(title = NULL),
      yaxis = list(title = "LFQ Values")
      )
})
```

```{r show bar-plot unlogded}
plotlyOutput("genePlotunlogged")
```
...
output$V1 <- renderPlotly({
    ...
    v <- ggplot(data=to_volcano, aes(x=FC_OCIAD1, y=-log10(p_OCIAD1_TOTALS), 
                                     color=sig_TOTALS_OCIAD1, label=gene)) + 
        geom_point() +
        geom_vline(xintercept=c(-1, 1), linetype = 2, alpha=0.5) +
        geom_hline(yintercept = -log10(0.05), linetype = 2, alpha=0.5) +
        labs(title='Volcano plot for Non-Imputed Data', x='log2(fold change)', 
             color='Significance', y='-log10(p-value)') +
        theme_bw()
    
    if (length(selected_row) != 0) {
        v + geom_point(data = sel_gene_data, 
                   aes(x=FC_OCIAD1, y=-log10(p_OCIAD1_TOTALS)), color = 'brown', size=3)
    }
...
to_volcano2 <- merge(FC,pvalue_imp)
to_volcano2$sig_TOTALS_OCIAD1 <- ifelse(abs(to_volcano2$FC_OCIAD1)>FC.cutoff&
                                           to_volcano2$p_OCIAD1_TOTALS<p.cutoff,
                                       yes = TRUE, no = FALSE)
```
...
output$V2 <- renderPlotly({
    selected_gene <- input$gene
    selected_row <- which(to_volcano2$gene == selected_gene)
    if (length(selected_row) != 0) {
        sel_gene_data <- to_volcano2 |> dplyr::filter(gene == selected_gene)
        to_volcano <- to_volcano2[-selected_row,]
    } else {
        to_volcano <- to_volcano2
    }
    ...
    v <- ggplot(data=to_volcano, aes(x=FC_OCIAD1, y=-log10(p_OCIAD1_TOTALS), 
        ...
        geom_vline(xintercept=c(-1, 1), linetype = 2, alpha=0.5) +
        geom_hline(yintercept = -log10(0.05), linetype = 2, alpha=0.5) +
        labs(title='Volcano plot for Imputed Data - Mati Code', x='log2(fold change)', 
             color='Significance', y='-log10(p-value)') +
        theme_bw()
...
means$missingness <- df_ml$missingness_per_prot[seq(1, nrow(imputed_long_RF), by=6)]

# Preparing data for volcano plot
FC <- merge(means, imputed_RF)
FC <- FC |> dplyr::mutate(gene = protein.groups$`Gene names`)

to_volcano3 <- merge(FC, pvalue_RF)
to_volcano3$sig_TOTALS_OCIAD1 <- ifelse(abs(to_volcano3$FC_OCIAD1)>FC.cutoff&
                                           to_volcano3$p_OCIAD1_TOTALS<p.cutoff,
                                       yes = TRUE, no = FALSE)
```
...
output$V3 <- renderPlotly({
    selected_gene <- input$gene
    selected_row <- which(to_volcano3$gene == selected_gene)
    if (length(selected_row) != 0) {
        sel_gene_data <- to_volcano3 |> dplyr::filter(gene == selected_gene)
        to_volcano <- to_volcano3[-selected_row,]
    } else {
        to_volcano <- to_volcano3
    }
    ...
    v <- ggplot(data=to_volcano, aes(x=FC_OCIAD1, y=-log10(p_OCIAD1_TOTALS), 
        ...
        geom_vline(xintercept=c(-1, 1), linetype = 2, alpha=0.5) +
        geom_hline(yintercept = -log10(0.05), linetype = 2, alpha=0.5) +
        labs(title='Volcano plot for Imputed Data - Random Forest', 
...
miss_type_kNN <- df_ml |> 
    ...
    reshape2::dcast(row ~ group, value.var='missingness_per_prot') |>
    dplyr::select(-row) |>
...
to_volcano4 <- merge(FC, pvalue_kNN)
to_volcano4$sig_TOTALS_OCIAD1 <- ifelse(abs(to_volcano4$FC_OCIAD1)>FC.cutoff&
                                           to_volcano4$p_OCIAD1_TOTALS<p.cutoff,
                                       yes = TRUE, no = FALSE)
```
...
output$V4 <- renderPlotly({
    selected_gene <- input$gene
    selected_row <- which(to_volcano4$gene == selected_gene)
    if (length(selected_row) != 0) {
        sel_gene_data <- to_volcano4 |> dplyr::filter(gene == selected_gene)
        to_volcano <- to_volcano4[-selected_row,]
    } else {
        to_volcano <- to_volcano4
    }
    
    v <- ggplot(data=to_volcano, 
                aes(x=FC_OCIAD1, y=-log10(p_OCIAD1_TOTALS), 
                shape=sig_TOTALS_OCIAD1, label=gene, color=missingness)) + 
        geom_point() +
        geom_vline(xintercept=c(-1, 1), linetype = 2, alpha=0.5) +
        geom_hline(yintercept = -log10(0.05), linetype = 2, alpha=0.5) +
...
to_volcano5$sig_TOTALS_OCIAD1 <- ifelse(abs(to_volcano5$FC_OCIAD1)>FC.cutoff&
                                       ...
                                       yes = TRUE, no = FALSE)
colnames(to_volcano5)[6] <- 'missingness' 
```
...
output$V5 <- renderPlotly({
    ...
    v <- ggplot(data=to_volcano, aes(x=FC_OCIAD1, y=-log10(p_OCIAD1_TOTALS), 
        ...
        geom_vline(xintercept=c(-1, 1), linetype = 2, alpha=0.5) +
        geom_hline(yintercept = -log10(0.05), linetype = 2, alpha=0.5) +
        labs(title='Volcano plot for Imputed Data - Ludovic', 
             x='log2(fold change)', color='Missing Type', y='-log10(p-value)', shape='Significance') +
        theme_bw()
...
to_volcano6 <- merge(FC,pvalue_eigen)
to_volcano6$sig_TOTALS_OCIAD1 <- ifelse(abs(to_volcano6$FC_OCIAD1)>FC.cutoff&
                                           to_volcano6$p_OCIAD1_TOTALS<p.cutoff,
                                       yes = TRUE, no = FALSE)
```
...
output$V1eig <- renderPlotly({
    selected_gene <- input$gene
    selected_row <- which(to_volcano6$gene == selected_gene)
    if (length(selected_row) != 0) {
        sel_gene_data <- to_volcano6 |> dplyr::filter(gene == selected_gene)
        to_volcano <- to_volcano6[-selected_row,]
    } else {
        to_volcano <- to_volcano6
    }
...
to_volcano7 <- merge(FC, pvalue_eigen_imp)
...
# Add a type of missing to the data frame with means for groups KO and WT 
means$missingness <- df_ml$missingness_per_prot[seq(1, nrow(eigen_df_long_RF), by=6)]

# Preparing data for volcano plot
FC <- merge(means, eigen_df_RF)
FC <- FC |> dplyr::mutate(gene = protein.groups$`Gene names`[eig_prots])

to_volcano8 <- merge(FC, pvalue_eigen_RF)
to_volcano8$sig_TOTALS_OCIAD1 <- ifelse(abs(to_volcano8$FC_OCIAD1)>FC.cutoff&
                                           to_volcano8$p_OCIAD1_TOTALS<p.cutoff,
                                       yes = TRUE, no = FALSE)
```

```{r v3 eigen}
# Plotting the volcano plot
output$V3eig <- renderPlotly({
    selected_gene <- input$gene
    selected_row <- which(to_volcano8$gene == selected_gene)
    if (length(selected_row) != 0) {
        sel_gene_data <- to_volcano8 |> dplyr::filter(gene == selected_gene)
        to_volcano <- to_volcano8[-selected_row,]
    } else {
        to_volcano <- to_volcano8
    }
...
to_volcano9 <- merge(FC, pvalue_eigen_kNN)
to_volcano9$sig_TOTALS_OCIAD1 <- ifelse(abs(to_volcano9$FC_OCIAD1)>FC.cutoff&
                                           to_volcano9$p_OCIAD1_TOTALS<p.cutoff,
                                       yes = TRUE, no = FALSE)
```

```{r v4 eigen}
# Plotting the volcano plot
output$V4eig <- renderPlotly({
    selected_gene <- input$gene
    selected_row <- which(to_volcano9$gene == selected_gene)
    if (length(selected_row) != 0) {
        sel_gene_data <- to_volcano9 |> dplyr::filter(gene == selected_gene)
        to_volcano <- to_volcano9[-selected_row,]
    } else {
        to_volcano <- to_volcano9
    }
...
to_volcano10 <- merge(FC, pvalue_eigen_ludovic)
to_volcano10$sig_TOTALS_OCIAD1 <- ifelse(abs(to_volcano10$FC_OCIAD1)>FC.cutoff&
                                           to_volcano10$p_OCIAD1_TOTALS<p.cutoff,
                                       yes = TRUE, no = FALSE)
```
...
output$V5eig <- renderPlotly({
    selected_gene <- input$gene
    selected_row <- which(to_volcano10$gene == selected_gene)
    if (length(selected_row) != 0) {
        sel_gene_data <- to_volcano10 |> dplyr::filter(gene == selected_gene)
        to_volcano <- to_volcano10[-selected_row,]
    } else {
        to_volcano <- to_volcano10
    }
...
6. [A comparative study of evaluating missing value imputation methods in label-free proteomics](https://www.nature.com/articles/s41598-021-81279-4)
