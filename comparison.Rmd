...
test <- which(rownames(eigen_df_ludovic) %in% rownames(eigen_df))

data <- data.frame(
    Gene_Name = protein.groups$`Gene names`[miss],
    Raw_data = lfq[miss,1:6],
    Eigen_raw = eigen_df,
    Raw_RF = imputed_RF[miss,1:6],
    Eigen_RF = LFQ_RF.eig_norm$norm_m[miss,],
    Raw_ludovic = imputed_ludovic[test,3:8],
    Eigen_ludovic = LFQ_ludovic.eig_norm$norm_m[test,]
)
```
...
output$genePlot <- renderPlotly({
  ...
  # Create the plot
  plot_ly(data = filtered_data, type = "scatter", mode = "markers") %>%
      add_bars(x = 1, y = ~Raw_data.KO.22, name = 'Raw Value') |>
      add_bars(x = 1, y = ~Eigen_raw.KO.22, name = 'Normalized Raw Value') |>
      add_bars(x = 1, y = ~Raw_RF.KO.22, 'Imputed Value (Method 1)') |>
      add_bars(x = 1, y = ~Eigen_RF.KO.22, name = 'Normalized Imputed Value (Method 1)') |>
      add_bars(x = 1, y = ~Raw_ludovic.KO_22, name = 'Imputed Value (Method 2)') |>
      add_bars(x = 1, y = ~Eigen_ludovic.KO_22, name = 'Normalized Imputed Value (Method 2)') |>
      add_bars(x = 2, y = ~Raw_data.KO.23, name = 'Raw Value') |>
      add_bars(x = 2, y = ~Eigen_raw.KO.23, name = 'Normalized Raw Value') |>
      add_bars(x = 2, y = ~Raw_RF.KO.23, 'Imputed Value (Method 1)') |>
      add_bars(x = 2, y = ~Eigen_RF.KO.23, name = 'Normalized Imputed Value (Method 1)') |>
      add_bars(x = 2, y = ~Raw_ludovic.KO_23, name = 'Imputed Value (Method 2)') |>
      add_bars(x = 2, y = ~Eigen_ludovic.KO_23, name = 'Normalized Imputed Value (Method 2)') |>
      add_bars(x = 3, y = ~Raw_data.KO.24, name = 'Raw Value') |>
      add_bars(x = 3, y = ~Eigen_raw.KO.24, name = 'Normalized Raw Value') |>
      add_bars(x = 3, y = ~Raw_RF.KO.24, 'Imputed Value (Method 1)') |>
      add_bars(x = 3, y = ~Eigen_RF.KO.24, name = 'Normalized Imputed Value (Method 1)') |>
      add_bars(x = 3, y = ~Raw_ludovic.KO_24, name = 'Imputed Value (Method 2)') |>
      add_bars(x = 3, y = ~Eigen_ludovic.KO_24, name = 'Normalized Imputed Value (Method 2)') |>
      layout(
      title = paste("Scatter Plot for Gene:", selected_gene),
      xaxis = list(title = "Raw / Imputed Values"),
