...
plotviolin <- function(df, xlab='', ylab='LFQValue') {
    ...
    df <- df %>%
        ...
        mutate(short.name = gsub("_TOTALS_", ".", Exp))
    
    violin <- ggplot(data=df, aes(x=short.name, y=LFQValue, fill=short.name))+
...
plotviolin(lfq, 'Original') + ggtitle('Comparison of Normalization Methods - Violin plots')
```
...
ggarrange(
    plotviolin(cbind(LFQ_KO.lm,LFQ_WT.lm), 'Linear Norm'),
    plotviolin(cbind(LFQ_ALL.vsn@assayData[['R']],LFQ_ALL.vsn@assayData[['G']]), 'VSN', ''),
    plotviolin(cbind(LFQ_KO.ordernorm,LFQ_WT.ordernorm), 'Order Quantile'),
...
cv_before <- apply(cbind(LFQ_KO, LFQ_WT), 1, cv)
...
cv.vsn <- apply(cbind(LFQ_ALL.vsn@assayData[['R']],LFQ_ALL.vsn@assayData[['G']]), 1, cv)
cv.ordernorm <- apply(cbind(LFQ_KO.ordernorm,LFQ_WT.ordernorm), 1, cv)
cv.eig <- apply(LFQ.eig_norm$norm_m, 1, cv)

cv_all <- data.frame(cv_before, cv.standard, cv.minmax, cv.med, cv.mad, cv.lm, cv.vsn, cv.ordernorm, cv.eig)
colnames(cv_all) <- c('Before', 'Z-score', 'MinMax', 'Median', 'MAD', 'Linear', 'VSN', 'OrderQuantile', 'EigenMS')

vplots1 <- ggarrange(
    plotoneviolin(cv_before, 'Original', 'red'),
...
ggarrange(
    ...
    plotoneviolin(cv.ordernorm, 'OrderQuantile', 'brown', 'brown'),
    nrow=2, ncol=3
)
```
...
```{r echo=FALSE}
cv_summary <- cv_all %>%
    pivot_longer(1:9, names_to = 'method', values_to = 'LFQ_CV') %>%
    group_by(method) %>%
    summarise('mean'=round(mean(LFQ_CV),5), 
              'median'=round(median(LFQ_CV),5), 
              'sd'=round(sd(LFQ_CV),5))

knitr::kable(cv_summary, format='html')
```
...
icc.vsn <- irr::icc(cbind(LFQ_ALL.vsn@assayData[['R']],LFQ_ALL.vsn@assayData[['G']]))$value
icc.ordernorm <- irr::icc(cbind(LFQ_KO.ordernorm,LFQ_WT.ordernorm))$value
...
rownames(icc_all) <- c('Before', 'Z-score', 'MinMax', 'Median', 'MAD', 'Linear', 'VSN', 'OrderQuantile', 'EigenMS')
