...
On the plots we can see the result of `svd()` fxn, which is:

The SVD decomposition of the matrix as computed by LAPACK,

$$X=UDV',$$

where $U$ and $V$ are orthogonal, $V'$ means *V transposed* (and conjugated for complex input), and 
$D$ is a diagonal matrix with the (non-negative) singular values $D_{ii}$ in decreasing order. Equivalently, 
$D=U'XV$, which is verified in the examples.
...
* **x-axis** - the number corresponding to the imputed samples in the order in which we imputed them.
* **y-axis** - the `u` value from the `svd()` fnx.
...
```{r echo=FALSE}
boxcox_df <- data.frame(LFQ_KO.boxcox, LFQ_WT.boxcox)
boxcox_df[is.na(boxcox_df)] <- 0

pvalue_boxcox <- boxcox_df |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(boxcox_df, 1, ttest, 
                  grp1=grep("KO", colnames(boxcox_df)), 
                  grp2=grep("WT", colnames(boxcox_df)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE))
```

```{r echo=FALSE}
hist_p_boxcox <- ggplot(data = pvalue_boxcox, aes(x = p_OCIAD1_TOTALS, fill=significant)) + geom_histogram(bins = 100)
```

```{r echo=FALSE}
mean_p_boxcox <- mean(pvalue_boxcox$p_OCIAD1_TOTALS, na.rm=TRUE)
```
...
```{r echo=FALSE}
hist_p_standard <- ggplot(data = pvalue_standard, aes(x = p_OCIAD1_TOTALS, fill=significant)) + geom_histogram(bins = 100)
```

```{r echo=FALSE}
mean_p_standard <- mean(pvalue_standard$p_OCIAD1_TOTALS, na.rm=TRUE)
```
...
```{r echo=FALSE}
hist_p_minmax <- ggplot(data = pvalue_minmax, aes(x = p_OCIAD1_TOTALS, fill=significant)) + geom_histogram(bins = 100)
```

```{r echo=FALSE}
mean_p_minmax <- mean(pvalue_minmax$p_OCIAD1_TOTALS, na.rm=TRUE)
```

## Median

```{r echo=FALSE}
median_df <- data.frame(LFQ_KO.med, LFQ_WT.med)
median_df[is.na(median_df)] <- 0

pvalue_med <- median_df |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(median_df, 1, ttest, 
                  grp1=grep("KO", colnames(median_df)), 
                  grp2=grep("WT", colnames(median_df)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE))
```

```{r echo=FALSE}
hist_p_med <- ggplot(data = pvalue_med, aes(x = p_OCIAD1_TOTALS, fill=significant)) + geom_histogram(bins = 100)
```

```{r echo=FALSE}
mean_p_med <- mean(pvalue_med$p_OCIAD1_TOTALS, na.rm=TRUE)
```
...
```{r echo=FALSE}
hist_p_mad <- ggplot(data = pvalue_mad, aes(x = p_OCIAD1_TOTALS, fill=significant)) + geom_histogram(bins = 100)
```

```{r echo=FALSE}
mean_p_mad <- mean(pvalue_mad$p_OCIAD1_TOTALS, na.rm=TRUE)
```
...
```{r echo=FALSE}
hist_p_lm <- ggplot(data = pvalue_lm, aes(x = p_OCIAD1_TOTALS, fill=significant)) + geom_histogram(bins = 100)
```

```{r echo=FALSE}
mean_p_lm <- mean(pvalue_lm$p_OCIAD1_TOTALS, na.rm=TRUE)
```

## VSN

```{r echo=FALSE}
vsn_df <- data.frame(LFQ.vsn$R, LFQ.vsn$G)
...
```{r echo=FALSE}
hist_p_vsn <- ggplot(data = pvalue_vsn, aes(x = p_OCIAD1_TOTALS, fill=significant)) + geom_histogram(bins = 100)
```

```{r echo=FALSE}
mean_p_vsn <- mean(pvalue_vsn$p_OCIAD1_TOTALS, na.rm=TRUE)
```

## EigenMS

```{r echo=FALSE}
eigen_df <- as.data.frame(LFQ.eig_norm$norm_m)
...
```{r echo=FALSE}
hist_p_eigen <- ggplot(data = pvalue_eigen, aes(x = p_OCIAD1_TOTALS, fill=significant)) + geom_histogram(bins = 100)
```

```{r echo=FALSE}
mean_p_eigen <- mean(pvalue_eigen$p_OCIAD1_TOTALS, na.rm=TRUE)
```

# p-value imputed Mati

## Box-Cox

```{r echo=FALSE}
boxcox_df_imp <- data.frame(LFQ_KO_imp.boxcox, LFQ_WT_imp.boxcox)
boxcox_df_imp[is.na(boxcox_df_imp)] <- 0

pvalue_boxcox <- boxcox_df_imp |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(boxcox_df_imp, 1, ttest, 
                  grp1=grep("KO", colnames(boxcox_df_imp)), 
                  grp2=grep("WT", colnames(boxcox_df_imp)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE))
```

```{r echo=FALSE}
hist_p_boxcox_imp <- ggplot(data = pvalue_boxcox, aes(x = p_OCIAD1_TOTALS, fill=significant)) + geom_histogram(bins = 100)
```

```{r echo=FALSE}
mean_p_boxcox_imp <- mean(pvalue_boxcox$p_OCIAD1_TOTALS, na.rm=TRUE)
```
...
standard_df_imp[is.na(standard_df_imp)] <- 0

pvalue_standard <- standard_df_imp |>
...
```{r echo=FALSE}
hist_p_standard_imp <- ggplot(data = pvalue_standard, aes(x = p_OCIAD1_TOTALS, fill=significant)) + geom_histogram(bins = 100)
```

```{r echo=FALSE}
mean_p_standard_imp <- mean(pvalue_standard$p_OCIAD1_TOTALS, na.rm=TRUE)
```
...
minmax_df_imp[is.na(minmax_df_imp)] <- 0

pvalue_minmax <- minmax_df_imp |>
...
```{r echo=FALSE}
hist_p_minmax_imp <- ggplot(data = pvalue_minmax, aes(x = p_OCIAD1_TOTALS, fill=significant)) + geom_histogram(bins = 100)
```

```{r echo=FALSE}
mean_p_minmax_imp <- mean(pvalue_minmax$p_OCIAD1_TOTALS, na.rm=TRUE)
```

## Median

```{r echo=FALSE}
median_df_imp <- data.frame(LFQ_KO_imp.med, LFQ_WT_imp.med)
median_df_imp[is.na(median_df_imp)] <- 0

pvalue_med <- median_df_imp |>
    dplyr::mutate("p_OCIAD1_TOTALS" = apply(median_df_imp, 1, ttest, 
                  grp1=grep("KO", colnames(median_df_imp)), 
                  grp2=grep("WT", colnames(median_df_imp)))) |>
    dplyr::mutate(significant = ifelse(p_OCIAD1_TOTALS < p.cutoff, TRUE, FALSE))
```

```{r echo=FALSE}
hist_p_med_imp <- ggplot(data = pvalue_med, aes(x = p_OCIAD1_TOTALS, fill=significant)) + geom_histogram(bins = 100)
```

```{r echo=FALSE}
mean_p_med_imp <- mean(pvalue_med$p_OCIAD1_TOTALS, na.rm=TRUE)
```
...
mad_df_imp[is.na(mad_df_imp)] <- 0

pvalue_mad <- mad_df_imp |>
...
```{r echo=FALSE}
hist_p_mad_imp <- ggplot(data = pvalue_mad, aes(x = p_OCIAD1_TOTALS, fill=significant)) + geom_histogram(bins = 100)
```

```{r echo=FALSE}
mean_p_mad_imp <- mean(pvalue_mad$p_OCIAD1_TOTALS, na.rm=TRUE)
```
...
```{r echo=FALSE}
hist_p_lm_imp <- ggplot(data = pvalue_lm, aes(x = p_OCIAD1_TOTALS, fill=significant)) + geom_histogram(bins = 100)
```

```{r echo=FALSE}
mean_p_lm_imp <- mean(pvalue_lm$p_OCIAD1_TOTALS, na.rm=TRUE)
```

## VSN

```{r echo=FALSE}
vsn_df_imp <- data.frame(LFQ_imp.vsn$R, LFQ_imp.vsn$G)
vsn_df_imp[is.na(vsn_df_imp)] <- 0

pvalue_vsn <- vsn_df_imp |>
...
```{r echo=FALSE}
hist_p_vsn_imp <- ggplot(data = pvalue_vsn, aes(x = p_OCIAD1_TOTALS, fill=significant)) + geom_histogram(bins = 100)
```

```{r echo=FALSE}
mean_p_vsn_imp <- mean(pvalue_vsn$p_OCIAD1_TOTALS, na.rm=TRUE)
```

## EigenMS

```{r echo=FALSE}
eigen_df_imp <- as.data.frame(LFQ_imp.eig_norm$norm_m)
eigen_df_imp[is.na(eigen_df_imp)] <- 0

pvalue_eigen <- eigen_df_imp |>
...
```{r echo=FALSE}
hist_p_eigen_imp <- ggplot(data = pvalue_eigen, aes(x = p_OCIAD1_TOTALS, fill=significant)) + geom_histogram(bins = 100)
```

```{r echo=FALSE}
mean_p_eigen_imp <- mean(pvalue_eigen$p_OCIAD1_TOTALS, na.rm=TRUE)
```
