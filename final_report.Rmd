...
plotviolin <- function(df, xlab='', ylab='LFQValue') {
    ...
    df <- df %>%
        pivot_longer(1:6,names_to = 'Exp', values_to = 'LFQValue') %>%
...
knitr::kable(head(lfq_imp), format='html')
```
...
```{r echo=FALSE}
ggarrange(
    plothist(cbind(LFQ.vsn$R, LFQ.vsn$G), 'Non-Imputed Transformed by VSN'),
    plothist(cbind(LFQ_imp.vsn$R, LFQ_imp.vsn$G), 'Imputed Transformed by VSN'),
    nrow=2,ncol=1
)
```
...
```{r autonorm KO 22}
best.22 <- bestNormalize::bestNormalize(LFQ_KO$KO.22) # Non-Imputed
best_imp.22 <- bestNormalize::bestNormalize(LFQ_KO_imp$KO.I.22) # Imputed
```

**Experiment 23**

```{r autonorm KO 23}
best.23 <- bestNormalize::bestNormalize(LFQ_KO$KO.23) # Non-Imputed
best_imp.23 <- bestNormalize::bestNormalize(LFQ_KO_imp$KO.I.23) # Imputed
```

**Experiment 24**

```{r autonorm KO 24}
best.24 <- bestNormalize::bestNormalize(LFQ_KO$KO.24) # Non-Imputed
best_imp.24 <- bestNormalize::bestNormalize(LFQ_KO_imp$KO.I.24) # Imputed
```
...
autonorm_ko <- data.frame(Exp_22=environment(best.22[["norm_stat_fn"]])[["best_idx"]],
                          Exp_22_imp=environment(best_imp.22[["norm_stat_fn"]])[["best_idx"]],
                          Exp_23=environment(best.23[["norm_stat_fn"]])[["best_idx"]],
                          Exp_23_imp=environment(best_imp.23[["norm_stat_fn"]])[["best_idx"]],
                          Exp_24=environment(best.24[["norm_stat_fn"]])[["best_idx"]],
                          Exp_24_imp=environment(best_imp.24[["norm_stat_fn"]])[["best_idx"]])
...
Order Quntile Normalization for samples:
...
For the rest of samples the best method is Yeo-Johnson.

```{r echo=FALSE}
LFQ_KO.22 <- bestNormalize::yeojohnson(LFQ_KO$KO.22)$x.t # Non-Imputed
LFQ_KO_imp.22 <- bestNormalize::orderNorm(LFQ_KO_imp$KO.I.22)$x.t # Imputed

LFQ_KO.23 <- bestNormalize::orderNorm(LFQ_KO$KO.23)$x.t # Non-Imputed
LFQ_KO_imp.23 <- bestNormalize::orderNorm(LFQ_KO_imp$KO.I.23)$x.t # Imputed

LFQ_KO.24 <- bestNormalize::yeojohnson(LFQ_KO$KO.24)$x.t # Non-Imputed
LFQ_KO_imp.24 <- bestNormalize::orderNorm(LFQ_KO_imp$KO.I.24)$x.t # Imputed

 # Non-Imputed
LFQ_KO.bestNorm <- data.frame('KO.22'=LFQ_KO.22,'KO.23'=LFQ_KO.23,'KO.24'=LFQ_KO.24)
 # Imputed
LFQ_KO_imp.bestNorm <- data.frame('KO.I.22'=LFQ_KO_imp.22,'KO.I.23'=LFQ_KO_imp.23,'KO.I.24'=LFQ_KO_imp.24)
...
```{r autonorm WT 22}
best.22 <- bestNormalize::bestNormalize(LFQ_WT$WT.22) # Non-Imputed
best_imp.22 <- bestNormalize::bestNormalize(LFQ_WT_imp$WT.I.22) # Imputed
```

**Experiment 23**

```{r autonorm WT 23}
best.23 <- bestNormalize::bestNormalize(LFQ_WT$WT.23) # Non-Imputed
best_imp.23 <- bestNormalize::bestNormalize(LFQ_WT_imp$WT.I.23) # Imputed
```

**Experiment 24**

```{r autonorm WT 24}
best.24 <- bestNormalize::bestNormalize(LFQ_WT$WT.24) # Non-Imputed
best_imp.24 <- bestNormalize::bestNormalize(LFQ_WT_imp$WT.I.24) # Imputed
```
...
```{r echo=FALSE}
LFQ_WT.22 <- bestNormalize::orderNorm(LFQ_WT$WT.22)$x.t # Non-Imputed
LFQ_WT_imp.22 <- bestNormalize::orderNorm(LFQ_WT_imp$WT.I.22)$x.t # Imputed

LFQ_WT.23 <- bestNormalize::orderNorm(LFQ_WT$WT.23)$x.t # Non-Imputed
LFQ_WT_imp.23 <- bestNormalize::orderNorm(LFQ_WT_imp$WT.I.23)$x.t # Imputed

LFQ_WT.24 <- bestNormalize::orderNorm(LFQ_WT$WT.24)$x.t # Non-Imputed
LFQ_WT_imp.24 <- bestNormalize::orderNorm(LFQ_WT_imp$WT.I.24)$x.t # Imputed

 # Non-Imputed
LFQ_WT.bestNorm <- data.frame('WT.22'=LFQ_WT.22,'WT.23'=LFQ_WT.23,'WT.24'=LFQ_WT.24)
 # Imputed
LFQ_WT_imp.bestNorm <- data.frame('WT.I.22'=LFQ_WT_imp.22,'WT.I.23'=LFQ_WT_imp.23,'WT.I.24'=LFQ_WT_imp.24)
...
cv.before <- apply(cbind(LFQ_KO, LFQ_WT), 1, cv)
cv.standard <- apply(cbind(LFQ_KO.standard,LFQ_WT.standard), 1, cv)
cv.minmax <- apply(cbind(LFQ_KO.minmax, LFQ_WT.minmax), 1, cv)
cv.med <- apply(cbind(LFQ_KO.med,LFQ_WT.med), 1, cv)
cv.mad <- apply(cbind(LFQ_KO.mad,LFQ_WT.mad), 1, cv)
cv.lm <- apply(cbind(LFQ_KO.lm,LFQ_WT.lm), 1, cv)
cv.vsn <- apply(cbind(LFQ.vsn$R,LFQ.vsn$G), 1, cv)
cv.bestnorm <- apply(cbind(LFQ_KO.bestNorm,LFQ_WT.bestNorm), 1, cv)
cv.eig <- apply(LFQ.eig_norm$norm_m, 1, cv)

vplots1 <- ggarrange(
    plotoneviolin(cv.before, 'Original', 'red'),
    plotoneviolin(cv.minmax, 'MinMax', 'green'),
    plotoneviolin(cv.vsn, 'VSN', 'orange'),
    plotoneviolin(cv.eig, 'EigenMS', 'blue'),
    nrow=2,ncol=2
)
...
```{r echo=FALSE}
ggarrange(
    plotoneviolin(cv.standard, 'Z-score', 'yellow', 'yellow'),
    plotoneviolin(cv.med, 'Median', 'magenta', 'magenta'),
    plotoneviolin(cv.mad, 'MAD', 'pink', 'pink'),
    plotoneviolin(cv.lm, 'Linear', 'purple', 'purple'),
    plotoneviolin(cv.bestnorm, 'bestNormalize Package', 'brown', 'brown'),
    nrow=2, ncol=3
)
```
...
colnames(cv_all) <- c('Before', 'Z-score', 'MinMax', 'Median', 'MAD', 'Linear', 'VSN', 'bestNormalize')

cv_summary <- cv_all %>%
    pivot_longer(1:8, names_to = 'method', values_to = 'LFQ_CV') %>%
    group_by(method) %>%
    summarise('mean'=round(mean(LFQ_CV, na.rm=TRUE),5), 
              'median'=round(median(LFQ_CV, na.rm=TRUE),5), 
              'sd'=round(sd(LFQ_CV, na.rm=TRUE),5))

summary_eigen <- tibble(cv.eig) %>%
    pivot_longer(1, names_to = 'method', values_to = 'LFQ_CV') %>%
    group_by(method) %>%
    summarise('mean'=round(mean(LFQ_CV, na.rm=TRUE),5), 
              'median'=round(median(LFQ_CV, na.rm=TRUE),5), 
...
knitr::kable(rbind(cv_summary, summary_eigen), format='html')
```
</center>
...
```{r cv imp, echo=FALSE}
cv_imp.before <- apply(cbind(LFQ_KO_imp, LFQ_WT_imp), 1, cv)
cv_imp.standard <- apply(cbind(LFQ_KO_imp.standard,LFQ_WT_imp.standard), 1, cv)
cv_imp.minmax <- apply(cbind(LFQ_KO_imp.minmax, LFQ_WT_imp.minmax), 1, cv)
cv_imp.med <- apply(cbind(LFQ_KO_imp.med,LFQ_WT_imp.med), 1, cv)
cv_imp.mad <- apply(cbind(LFQ_KO_imp.mad,LFQ_WT_imp.mad), 1, cv)
cv_imp.lm <- apply(cbind(LFQ_KO_imp.lm,LFQ_WT_imp.lm), 1, cv)
cv_imp.vsn <- apply(cbind(LFQ_imp.vsn$R,LFQ_imp.vsn$G), 1, cv)
cv_imp.bestnorm <- apply(cbind(LFQ_KO_imp.bestNorm,LFQ_WT_imp.bestNorm), 1, cv)
cv_imp.eig <- apply(LFQ_imp.eig_norm$norm_m, 1, cv)

vplots2 <- ggarrange(
    plotoneviolin(cv_imp.before, 'Original', 'red'),
    plotoneviolin(cv_imp.minmax, 'MinMax', 'green'),
    plotoneviolin(cv_imp.vsn, 'VSN', 'orange'),
    plotoneviolin(cv_imp.eig, 'EigenMS', 'blue'),
    nrow=2,ncol=2
)
...
```{r echo=FALSE}
ggarrange(
    plotoneviolin(cv_imp.standard, 'Z-score', 'yellow', 'yellow'),
    plotoneviolin(cv_imp.med, 'Median', 'magenta', 'magenta'),
    plotoneviolin(cv_imp.mad, 'MAD', 'pink', 'pink'),
    plotoneviolin(cv_imp.lm, 'Linear', 'purple', 'purple'),
    plotoneviolin(cv_imp.bestnorm, 'bestNormalize Package', 'brown', 'brown'),
    nrow=2, ncol=3
)
```
...
cv_all <- data.frame(cv_imp.before, cv_imp.standard, cv_imp.minmax, cv_imp.med,
                     cv_imp.mad, cv_imp.lm, cv_imp.vsn, cv_imp.bestnorm)
colnames(cv_all) <- c('Before', 'Z-score', 'MinMax', 'Median', 'MAD', 'Linear', 'VSN', 'bestNormalize')

cv_imp_summary <- cv_all %>%
    pivot_longer(1:8, names_to = 'method', values_to = 'LFQ_CV') %>%
    group_by(method) %>%
    summarise('mean'=round(mean(LFQ_CV, na.rm=TRUE),5), 
              'median'=round(median(LFQ_CV, na.rm=TRUE),5), 
...
summary_eigen <- tibble(cv_imp.eig) %>%
    pivot_longer(1, names_to = 'method', values_to = 'LFQ_CV') %>%
    group_by(method) %>%
    summarise('mean'=round(mean(LFQ_CV, na.rm=TRUE),5), 
              'median'=round(median(LFQ_CV, na.rm=TRUE),5), 
...
knitr::kable(rbind(cv_imp_summary, summary_eigen), format='html')
```
</center>
...
```{r icc, echo=FALSE}
icc_before <- irr::icc(lfq)$value
icc.standard <- irr::icc(cbind(LFQ_KO.standard,LFQ_WT.standard))$value
icc.minmax <- irr::icc(cbind(LFQ_KO.standard,LFQ_WT.standard))$value
icc.med <- irr::icc(cbind(LFQ_KO.med,LFQ_WT.med))$value
icc.mad <- irr::icc(cbind(LFQ_KO.mad,LFQ_WT.mad))$value
icc.lm <- irr::icc(cbind(LFQ_KO.lm,LFQ_WT.lm))$value
icc.vsn <- irr::icc(cbind(LFQ.vsn$R,LFQ.vsn$G))$value
icc.ordernorm <- irr::icc(cbind(LFQ_KO.bestNorm,LFQ_WT.bestNorm))$value
icc.eig <- irr::icc(LFQ.eig_norm$norm_m)$value

icc_all <- data.frame(ICC=c(icc_before, icc.standard, icc.minmax, icc.med, 
                            icc.mad, icc.lm, icc.vsn, icc.ordernorm, icc.eig))
rownames(icc_all) <- c('Before', 'Z-score', 'MinMax', 'Median', 'MAD', 'Linear', 
...
```{r icc imp, echo=FALSE}
icc_before_imp <- irr::icc(lfq_imp)$value
icc.standard_imp <- irr::icc(cbind(LFQ_KO_imp.standard,LFQ_WT_imp.standard))$value
icc.minmax_imp <- irr::icc(cbind(LFQ_KO_imp.standard,LFQ_WT_imp.standard))$value
icc.med_imp <- irr::icc(cbind(LFQ_KO_imp.med,LFQ_WT_imp.med))$value
icc.mad_imp <- irr::icc(cbind(LFQ_KO_imp.mad,LFQ_WT_imp.mad))$value
icc.lm_imp <- irr::icc(cbind(LFQ_KO_imp.lm,LFQ_WT_imp.lm))$value
icc.vsn_imp <- irr::icc(cbind(LFQ_imp.vsn$R,LFQ_imp.vsn$G))$value
icc.ordernorm_imp <- irr::icc(cbind(LFQ_KO_imp.bestNorm,LFQ_WT_imp.bestNorm))$value
icc.eig_imp <- irr::icc(LFQ_imp.eig_norm$norm_m)$value

icc_all_imp <- data.frame(ICC=c(icc_before_imp, icc.standard_imp, icc.minmax_imp, 
                                icc.med_imp, icc.mad_imp, icc.lm_imp, icc.vsn_imp, 
                                icc.ordernorm_imp, icc.eig_imp))
rownames(icc_all_imp) <- c('Before', 'Z-score', 'MinMax', 'Median', 'MAD', 'Linear', 
                       'VSN', 'bestNormalize', 'EigenMS')
```

<center>
```{r echo=FALSE}
knitr::kable(icc_all_imp, format='html')
```
</center>
